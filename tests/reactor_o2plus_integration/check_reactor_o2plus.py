...
def load(csv_path):
    ...
    rf = max(rows, key=lambda r: float(r["time"]))
    return r0, rf
...
def correction_off(csv_path):
    _, r1 = load(csv_path)
    sum_err = max(abs(float(r1["sum_w_min"]) - 1.0), abs(float(r1["sum_w_max"]) - 1.0))
    positive = float(r1["ion_min"]) >= -1e-12 and float(r1["neutral_min"]) >= -1e-12
    ok = sum_err > 1e-6 and positive
    print(f"sum_w_err={sum_err:.6e}")
    print("CORRECTION_OFF_NEGATIVE_CONTROL:", "PASS" if ok else "FAIL")
    return ok
...
def poisson_analytic(csv_path):
    ...
    rho = 1e-9
    w = 1e-6
    M = 0.032003320316217242
    S = e * rho * w * NA / (M * eps0)
    phi_max_exact = S / 8.0
    phi_avg_exact = S / 12.0
    w_err = rel(float(r["w_avg"]), w)
    phi_max_err = rel(float(r["phi_max"]), phi_max_exact)
    phi_avg_err = rel(float(r["phi_avg"]), phi_avg_exact)
...
def self_consistent(csv_path):
    r0, r1 = load(csv_path)
    m0 = float(r0["ion_mass"])
    m1 = float(r1["ion_mass"])
    n0 = float(r0["neutral_mass"])
    n1 = float(r1["neutral_mass"])
    ion_mass_err = rel(m1, m0)
    neutral_mass_err = rel(n1, n0)
    sum_err = max(abs(float(r1["sum_w_min"]) - 1.0), abs(float(r1["sum_w_max"]) - 1.0))
    ...
    phi_positive = float(r1["phi_max"]) > 0.0
    direction = c1 < c0
    ok = ion_mass_err < 1e-8 and neutral_mass_err < 1e-8 and sum_err < 1e-8 and positive and phi_positive and direction
    print(f"ion_mass_err={ion_mass_err:.6e}")
    print(f"neutral_mass_err={neutral_mass_err:.6e}")
    print(f"sum_w_err={sum_err:.6e}")
...
if __name__ == "__main__":
    ...
    checks = {
        ...
        "poisson_analytic": poisson_analytic,
