...
LEGACY_NAMESPACE = "qpx_harness.diagnose"
LEGACY_DIR = REPO_ROOT / "qpx_harness" / "diagnose"
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if (is_builtin_import or is_importlib) and (
                first.value == LEGACY_NAMESPACE
                or first.value.startswith(LEGACY_NAMESPACE + ".")
            ):
                hits.append(f"dynamic import {first.value}")
    return hits


def test_diagnose_namespace_is_physically_retired() -> None:
    assert not LEGACY_DIR.exists(), "qpx_harness/diagnose must remain physically retired"


def test_no_active_code_imports_retired_diagnose_namespace() -> None:
    violations: list[str] = []
    ...
    assert not violations, "retired diagnose imports remain:\n" + "\n".join(violations)
