...
print(f"expected from surface only         = {-surf*dt:.12e}")
print(f"expected from migration only       = {-mig*dt:.12e}")
print(f"expected from total wall           = {-wall*dt:.12e}")
print()
...
print(f"phi avg/min/max                    = {float(r1['phi_avg']):.12e} / "
      f"{float(r1['phi_min']):.12e} / {float(r1['phi_max']):.12e}")
...
if mode == "migration_only":
    ok = (
        ...
        and rm > 0
        and abs(lm) < 1e-30
        and rel(dm, -mig*dt) < 1e-8
    )
else:
    ok = (
        rm > 0
        and abs(lm) < 1e-30
        and rel(dm, -wall*dt) < 1e-8
    )
