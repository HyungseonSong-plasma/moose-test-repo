...
SCHEMA_VERSION=1
RESOURCES={"file","branch","issue"}
ACTIONS={"file":{"create","update","delete"},"branch":{"create","move","delete"},"issue":{"update","close","reopen"}}
...
def _branch(v,w):
    ...
    return v
def _updated(v,w):
    if not isinstance(v,str) or not v.endswith("Z") or "T" not in v: raise ManifestError(f"{w} must be GitHub updated_at timestamp")
    return v
...
def parse_manifest(raw:Any)->MutationManifest:
    ...
    if resource=="file":
        ...
        if action=="create":
            ...
            _keys(expected,{"sha"},{"sha"},"expected"); _sha(expected["sha"],"expected.sha"); _keys(desired,{"sha"},{"sha"},"desired"); _sha(desired["sha"],"desired.sha")
        else:
            _keys(expected,{"sha"},{"sha"},"expected"); _sha(expected["sha"],"expected.sha"); _keys(desired,set(),set(),"desired")
        if msg is not None: raise ManifestError("branch mutations do not accept commit_message")
    else:
        _keys(target,{"number"},{"number"},"target")
        if not isinstance(target["number"],int) or target["number"]<=0: raise ManifestError("target.number must be positive")
        _keys(expected,{"updated_at"},{"updated_at"},"expected"); _updated(expected["updated_at"],"expected.updated_at")
        if action=="update":
            _keys(desired,{"title","body"},set(),"desired")
            if not desired: raise ManifestError("issue update requires title/body")
        else: _keys(desired,set(),set(),"desired")
        if msg is not None: raise ManifestError("issue mutations do not accept commit_message")
    return MutationManifest(repo,resource,action,target,expected,desired,msg)

def load_manifest(path): return parse_manifest(json.loads(Path(path).read_text(encoding="utf-8")))
    ...
    def _call(self,method,path,payload=None,query=None):
        ...
        try:
            with urlopen(req,timeout=30) as r:
                b=r.read(); return None if not b else json.loads(b.decode())
        except HTTPError as e:
            body=e.read().decode(errors="replace")
            try: detail=json.loads(body).get("message",body)
            except json.JSONDecodeError: detail=body
            x=HardStop(f"GitHub API {method} {path} -> {e.code}: {detail}"); setattr(x,"status",e.code); raise x
...
class Engine:
    ...
    def _file(self,m):
        ...
        if m.action=="create":
            if cur is not None: raise HardStop("file create expected absence")
            self.t.request("PUT",api,payload={"message":m.commit_message,"content":base64.b64encode(m.desired["content"].encode()).decode(),"branch":b})
        else:
            if cur is None: raise HardStop(f"file {m.action} target absent")
            ...
            if m.action=="update":
                ...
                if old==m.desired["content"]: return self._noop(m,cur["sha"])
                self.t.request("PUT",api,payload={"message":m.commit_message,"content":base64.b64encode(m.desired["content"].encode()).decode(),"branch":b,"sha":cur["sha"]})
            else:
                ...
                return self._ok(m)
        v=self.t.get(api,query={"ref":b}); actual=base64.b64decode(v["content"]).decode()
        if actual!=m.desired["content"]: raise HardStop("post-write verification mismatch for file content")
        return self._ok(m,v.get("sha"))
    def _branch(self,m):
        n=m.target["name"]; api="/git/ref/heads/"+quote(n,safe="/"); cur=self._read(api)
        if m.action=="create":
            if cur is not None: raise HardStop("branch create expected absence")
            sha=m.desired["sha"]; self.t.request("POST","/git/refs",payload={"ref":f"refs/heads/{n}","sha":sha})
        else:
            if cur is None: raise HardStop(f"branch {m.action} target absent")
            old=cur["object"]["sha"]
            if old!=m.expected["sha"]: raise HardStop(f"stale branch identity: actual={old} expected={m.expected['sha']}")
            if m.action=="delete":
                self.t.request("DELETE","/git/refs/heads/"+quote(n,safe="/"))
                if self._read(api) is not None: raise HardStop("post-write verification mismatch: branch still exists")
                return self._ok(m)
            sha=m.desired["sha"]
            if sha==old: return self._noop(m,old)
            c=self.t.get(f"/compare/{old}...{sha}")
            if c.get("status") not in {"ahead","identical"} or int(c.get("behind_by",0))!=0: raise HardStop("non-fast-forward branch move rejected")
            self.t.request("PATCH","/git/refs/heads/"+quote(n,safe="/"),payload={"sha":sha,"force":False})
        v=self.t.get(api); got=v["object"]["sha"]; want=m.desired["sha"]
        if got!=want: raise HardStop("post-write verification mismatch for branch")
        return self._ok(m,got)
    def _issue(self,m):
        api=f"/issues/{m.target['number']}"; cur=self.t.get(api)
        if cur.get("updated_at")!=m.expected["updated_at"]: raise HardStop("stale issue identity")
        if m.action=="update":
            if all(cur.get(k)==v for k,v in m.desired.items()): return self._noop(m,cur.get("updated_at"))
            payload=m.desired
        else:
            state="closed" if m.action=="close" else "open"
            if cur.get("state")==state: return self._noop(m,cur.get("updated_at"))
            payload={"state":state}
        self.t.request("PATCH",api,payload=payload); v=self.t.get(api)
        for k,val in payload.items():
            if v.get(k)!=val: raise HardStop(f"post-write verification mismatch for issue field {k}")
        return self._ok(m,v.get("updated_at"))

class Fake:
    def __init__(self): self.files={}; self.branches={}; self.issues={}; self.runs=[]; self.ff=set(); self.break_verify=False
    ...
    def get(self,path,*,query=None):
        ...
        if path.startswith("/compare/"):
            a,b=path.removeprefix("/compare/").split("...",1)
            return {"status":"ahead","behind_by":0} if (a,b) in self.ff else {"status":"diverged","behind_by":1}
        if path.startswith("/issues/"): return dict(self.issues[int(path.rsplit("/",1)[1])])
    ...
    def request(self,method,path,*,payload=None):
        ...
        if path.startswith("/git/refs/heads/"):
            n=path.removeprefix("/git/refs/heads/")
            if method=="DELETE": self.branches.pop(n,None)
            else: self.branches[n]=payload["sha"]
            return {}
        if path.startswith("/issues/"):
            n=int(path.rsplit("/",1)[1]); self.issues[n].update(payload); self.issues[n]["updated_at"]="2026-09-18T15:00:01Z"; return {}
...
def self_test():
    ...
    sha=f.files[("x.txt","main")]["sha"]; assert e.execute(M("file","update",{"path":"x.txt","branch":"main"},{"sha":sha},{"content":"new"},"noop"))["status"]=="NO_MUTATION_NEEDED"
    try: e.execute(M("file","update",{"path":"x.txt","branch":"main"},{"sha":old},{"content":"x"},"stale")); raise AssertionError
    ...
    try: M("issue","delete",{"number":1},{"updated_at":"2026-09-18T15:00:00Z"},{}); raise AssertionError
    except ManifestError: pass
    b=Fake(); b.branches["main"]=old
    try: Engine(b,repository="o/r").execute(M("branch","move",{"name":"main"},{"sha":old},{"sha":new})); raise AssertionError
    except HardStop: pass
    b.ff.add((old,new)); assert Engine(b,repository="o/r").execute(M("branch","move",{"name":"main"},{"sha":old},{"sha":new}))["status"]=="PASS"
    i=Fake(); i.issues[7]={"title":"old","body":"","state":"open","updated_at":"2026-09-18T15:00:00Z"}; ie=Engine(i,repository="o/r")
    assert ie.execute(M("issue","update",{"number":7},{"updated_at":"2026-09-18T15:00:00Z"},{"title":"new"}))["status"]=="PASS"
...
def main(argv=None):
    ...
    if not token: p.error(f"{a.token_env} required")
    try:
        ...
        if a.result: Path(a.result).write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
        return 0
    except MutationError as e:
        result={"status":"HARD_STOP","error":str(e)}; print("REPOSITORY_MUTATION=HARD_STOP",file=sys.stderr); print(json.dumps(result,sort_keys=True),file=sys.stderr)
        if a.result: Path(a.result).write_text(json.dumps(result,indent=2,sort_keys=True)+"\n")
        return 2
