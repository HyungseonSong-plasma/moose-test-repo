...
\`\`\`bash
python3 tools/workflow/new_work.py --issue 253 --kind experiments --title "Gummel dt release"
python3 tools/workflow/new_work.py --issue 254 --kind refactor --title "Profile-step parser repair"
\`\`\`
...
\`tools/workflow/run_work.py\`; nonzero exits, timeouts, malformed manifests, identity
