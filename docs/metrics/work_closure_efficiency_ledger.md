...
| qpx-lazy-cli-startup-isolation | #115 Decouple legacy CLI command imports from canonical gateway startup | C2 | 3 | 2 | 0 | 0 | 0/3 | 0 | 0 | 0 | n/a | no | canonical gateway startup coupled to unrelated legacy imports | integration work should be accepted on current main by semantic ownership; do not duplicate already-integrated branch work | satisfied by #120 current-main lazy CLI routing; final main CI SUCCESS; scientific P3/EVR 0 |
| physics-public-artifact-ci-cutover | #182 Publish physics-build-base / physics-runtime artifacts for public CI cutover | C4 | unavailable | unavailable | unavailable | unavailable | n/a | unavailable | unavailable | unavailable | n/a | no | planned cross-layer CI infrastructure / immutable artifact cutover | expensive dependency builds should remain immutable producer events; ordinary CI should consume a minimized immutable runtime and measure transfer/startup cost independently from scientific execution | public credential-free pull PASS; scratch runtime ~517 MB with 6 s cold pull; check-input + bounded real runtime PASS; ordinary CI cut over; no scientific V&V claim |
| physics-stale-harness-owner-migration | #186 Complete remaining stale harness import-owner migration (5 files) | C2 | unavailable | unavailable | unavailable | 0 | 0/3 | unavailable | unavailable | unavailable | n/a | no | stale canonical import ownership / dependency-gated collection evidence | bounded static cleanliness is not closure evidence; require ordinary CI pytest collection once the global gate reopens | five import-owner migrations preserved semantics; final full CI and direct pytest collection/execution PASS with zero retired-namespace collection failures; no scientific P3 |
| physics-zero-legacy-group-c | #190 Zero-legacy cleanup Group C — historical guard/convergence migration (14 files) | C4 | unavailable | unavailable | unavailable | 0 | 0/3 | unavailable | unavailable | unavailable | n/a | no | residual legacy ownership plus latent post-zero CI/test/bootstrap assumptions | zero-legacy cutover must validate active CI, pytest/bootstrap paths, and canonical symbols after the census reaches zero, not only import counts | external legacy imports 14→0; strict zero-legacy, active CI surface, full pytest, internal validation, architecture guards and immutable runtime smoke PASS; no scientific semantic rewrite |
...
For #186 and #190, prospective per-Issue WCC/T-WCC/RVR/DBR/RWR/CLR counters were not preserved. Those fields are intentionally `unavailable` rather than reconstructed from incomplete interaction history. `EVR=0` and `0/3` are retained because neither work item used a user-local QPX execution return; closure was established through ordinary GitHub Actions CI evidence.
