...
The v2 generator supplied `Te/ne` only when both selected species were charged. `PhysicsThermalDiffusionMaterial` validates all active `i <= j` pairs, so `Op + O2` still includes `Op|Op` and `Om + O2` includes `Om|Om`. Those charged self-pairs require `electron_temperature` and `electron_number_density`.
