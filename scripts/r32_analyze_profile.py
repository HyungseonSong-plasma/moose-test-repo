#!/usr/bin/env python3
"""Analyze issue #32 QPX one-step profiling evidence deterministically."""
...
def analyze(summary_path: Path, petsc_path: Path, perf_path: Path | None = None) -> dict:
    ...
    return {
        ...
        "dofs": int(float(metrics["r32_num_dofs"])) if metrics.get("r32_num_dofs") else None,
        "nonlinear_iterations": int(float(metrics["r32_nonlinear_iterations"]))
        if metrics.get("r32_nonlinear_iterations")
        else None,
        "linear_iterations": int(float(metrics["r32_linear_iterations"]))
        if metrics.get("r32_linear_iterations")
        else None,
        "residual_evaluations": int(float(metrics["r32_residual_evaluations"]))
        if metrics.get("r32_residual_evaluations")
        else None,
        "petsc_seconds": {
            "snes_solve": snes,
