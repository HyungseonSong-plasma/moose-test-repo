"""Reusable structural inspection of MOOSE C++ functor declarations and calls."""
...
from .calls import split_call_arguments
from .source import CppSource, CppSourceError

CPP_TEXT_SUFFIXES = {".C", ".cc", ".cpp", ".cxx", ".h", ".hh", ".hpp"}


class FunctorInspectionError(CppSourceError):
    """Raised when a requested functor structure cannot be resolved uniquely."""
...
def extract_functor_property_declaration(
    ...
    *,
    ...
    """Describe one ``addFunctorProperty`` call selected by a marker token."""
    ...
    if len(calls) != 1:
        raise FunctorInspectionError(
            f"expected exactly one {property_marker} addFunctorProperty declaration, "
            f"found {len(calls)}"
        )
...
def parameter_functor_calls(
    ...
    """Find spatial arguments used to call one parameter-backed functor."""
