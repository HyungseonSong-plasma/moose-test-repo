"""Typed models for registry- and Z3-driven numerical diagnosis."""
...
class DiagnosisRule(BaseModel):
    """Legacy single-metric threshold rule retained for compatibility."""
...
class DiagnosisRuleRegistry(BaseModel):
    """Metric registry plus legacy deterministic threshold rules."""
...
class MetricPredicate(BaseModel):
    """Declarative scalar comparison consumed by the generic Z3 engine."""
...
class Z3OwnerRule(BaseModel):
    """Externally injectable logical owner rule.

    ``all_of`` conditions must all hold, ``any_of`` requires at least one when
    provided, and ``none_of`` conditions must all be false. Priority is applied
    by the Z3 engine to select exactly one owner when multiple candidates hold.
    """
...
class Z3RuleSet(BaseModel):
    """Externally injectable ordered logical policy for owner attribution."""
...
class Z3Decision(BaseModel):
    """Logical result returned by the SMT decision layer."""
...
class DiagnosisReport(BaseModel):
    """Validated machine-readable result from numerical and logical diagnosis."""
