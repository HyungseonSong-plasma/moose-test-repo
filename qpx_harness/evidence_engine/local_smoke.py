"""Run the Polars + DuckDB evidence engine locally without QPX or MOOSE.

Usage:

    python -m qpx_harness.evidence_engine.local_smoke
    python -m qpx_harness.evidence_engine.local_smoke --output local_evidence_smoke

The first form uses a temporary directory. The second preserves Parquet,
DuckDB, and JSON artifacts for interactive inspection.
"""
...
def run_smoke(output: Path | None = None) -> dict[str, Any]:
    """Execute the complete local evidence-engine smoke contract."""
    if output is not None:
        output = output.resolve()
        output.mkdir(parents=True, exist_ok=True)
        return _run_at(output)

    with tempfile.TemporaryDirectory(prefix="qpx_evidence_smoke_") as temp:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="preserve Parquet/DuckDB/summary artifacts in this directory",
    )
    parser.add_argument(
        "--json",
        action="store_true",
        help="print the full JSON summary instead of the compact status",
    )
    ...
    try:
        ...
        print(f"EVIDENCE_ENGINE_LOCAL_SMOKE: FAIL ({type(exc).__name__}: {exc})")
        return 1

    if args.json:
        print(json.dumps(summary, indent=2, sort_keys=True))
    else:
        print(f"EVIDENCE_ENGINE_LOCAL_SMOKE: {summary['status']}")
