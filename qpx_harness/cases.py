...
def purge_generated_artifacts(
    ...
    *,
    ...
    """Remove caller-declared generated artifacts below ``root``.

    Directory names and glob patterns are caller-owned policy. The primitive
    returns deterministic relative-path facts for the entries it removed.
    """
    root = Path(root)
    if not root.is_dir():
...
def stage_case(
    ...
    *,
    ...
    """Copy a case tree, purge declared generated artifacts, and optionally replace input."""
    source = Path(source)
    target = Path(target)
...
def referenced_file_parameters(
    ...
    *,
    ...
    """Resolve MOOSE-style ``file`` and ``*_file`` parameter references mechanically."""
...
def validate_referenced_files(
    ...
    *,
    ...
    """Return resolved input-file references or raise when any referenced file is missing."""
...
def validate_case_references(
    ...
    *,
    ...
    """Read one direct-child input file and validate its referenced assets."""
    case_dir = Path(case_dir)
    safe_input_name = _direct_child_name(input_name, label="input name")
