...
def _telemetry_adapter(state_callback: StateCallback | None, *, stall_after_seconds: float) -> TelemetryCallback | None:
...
def prepare_temporal_outputs(case_dir: Path, cfg: dict, *, reporter: Reporter) -> None:
...
def _run_prepare(
    ...
    *,
    ...
    result = run_command(
        ...
        telemetry_callback=_telemetry_adapter(state_callback, stall_after_seconds=stall_after_seconds),
...
def run_case(
    ...
    *,
    ...
    active_reporter.case_started(case_dir=case_dir, test_type=test_type, input_name=input_name, log_path=log_path)
    ...
    if not checker:
        ...
        return 0
    return _run_checker(case_dir, checker=checker, checker_args=checker_args, result_dir=result_dir, reporter=active_reporter)
...
def run_suite(
    ...
    *,
    ...
    manifests = discover_manifests(roots, on_missing_root=active_reporter.root_skipped)
    selected: list[tuple[Path, str, Path]] = []
    for manifest in manifests:
        test_type = manifest_type(manifest)
        if requested_type == "all" or test_type == requested_type:
            owner_root = next((root for root in roots if root == manifest or root in manifest.parents), manifest.parent)
    ...
    for index, (manifest, _test_type, owner_root) in enumerate(selected, start=1):
        ...
        status = ExecutionState.SUCCEEDED if rc == 0 else ExecutionState.FAILED
        emit(status, wall_seconds)
        cases.append(CaseSummary(name=case_name, path=str(case_dir), status=status, wall_seconds=wall_seconds))
        if rc != 0:
...
def cli_run_all(argv: list[str] | None = None) -> int:
    ...
    if not roots:
        ...
        if not default.exists():
            raise SystemExit(f"default tests root does not exist: {default}; provide --tests-root")
        roots = [default]
    result = run_suite(
        roots,
