...
def _build_electron_300k(base_text: str, *, dt: float, steps: int) -> str:
    try:
        text, _ = MooseInput(base_text).replace_parameters(
            ...
            {
                "prop_values": (
                    f"'{_fmt(v1.MEAN_ENERGY_EV)} {_fmt(DEFAULT_PRESSURE)} "
...
def _build_oneway(base_text: str, *, dt: float, steps: int, radial_span: float) -> str:
    text, _ = v1.build_fast_input(
        base_text,
    ...
    if "potential = potential_plasma" in MooseInput(text).text[
        MooseInput(text).unique("FVKernels/drift").start : MooseInput(text).unique("FVKernels/drift").end
    ]:
        raise FastPlasmaV2Error("one-way case retained phi->electron feedback")
    return text


def _build_feedback(base_text: str, *, dt: float, steps: int, radial_span: float) -> str:
    text, _ = v1.build_fast_input(
        base_text,
        gas_temperature=GAS_TEMPERATURE,
        electron_density=DEFAULT_ELECTRON_DENSITY,
...
def _residual_summary(log_path: str | None) -> dict[str, Any] | None:
    ...
    pattern = re.compile(
        r"(?m)^\s*(\d+)\s+Nonlinear\s+\|R\|\s*=\s*([0-9.+\-eE]+)"
    )
...
def _run_case(
    *,
    ...
    v1._copy_case(base_case, case_dir, input_text)
    v1._validate_assets(case_dir)
    validate_parser_symbols_text(input_text)
    result = v1._run_qpx_case(
        case_dir=case_dir,
        case_id=case_id,
        exe=exe,
...
def classify(
    ...
    if feedback_base is None:
        return {"class": "FEEDBACK_CASE_MISSING", "reason": "base feedback discriminator missing"}
...
def self_test() -> int:
    try:
        if v1.self_test() != 0:
            raise AssertionError("v1 self-test failed")
        sample = """ 0 Nonlinear |R| = 1.0e+03\n 1 Nonlinear |R| = 1.0e+01\n 2 Nonlinear |R| = 2.0e+00\n"""
        import tempfile
...
def main(argv: list[str] | None = None) -> int:
    ...
    base_case = repo_root / v1.BASE_CASE_RELATIVE
    ...
    root = v1._create_root(results_root)
    root = root.with_name(root.name.replace("fast_plasma_relaxation_", "fast_plasma_discriminator_v2_"))
    # _create_root already made the original directory; rename it atomically.
    original = next((p for p in root.parent.glob(root.name.replace("fast_plasma_discriminator_v2_", "fast_plasma_relaxation_") + "*")), None)
    ...
    if known_good.get("class") != "P3_PASS" or known_good.get("canonical_checker", {}).get("status") != "PASS":
        ...
        _write_json(summary_path, {"scale_map": scales, "known_good": known_good, "decision": decision})
    ...
    e300_text = _build_electron_300k(base_text, dt=DT_ELECTRON_CONTROL, steps=N_STEPS)
    ...
    if _physics_pass(electron_300k):
        oneway_text = _build_oneway(
            base_text, dt=DT_ELECTRON_CONTROL, steps=N_STEPS, radial_span=radial_span
        )
        oneway = _run_case(
            base_case=base_case,
    ...
    if oneway is not None and _physics_pass(oneway):
        feedback_text = _build_feedback(
            base_text, dt=DT_FEEDBACK_BASE, steps=N_STEPS, radial_span=radial_span
        )
        ...
        if _physics_pass(feedback_base):
            large_text = _build_feedback(
                base_text, dt=DT_FEEDBACK_LARGE, steps=N_STEPS, radial_span=radial_span
            )
            ...
            small_text = _build_feedback(
                base_text, dt=DT_FEEDBACK_SMALL, steps=N_STEPS, radial_span=radial_span
            )
    ...
    if feedback_base is not None:
        ...
        print(f"ISSUE43_FAST2_FEEDBACK_1E13_RESIDUAL: {feedback_base.get('nonlinear_residual_summary')}")
    if feedback_small is not None:
        print(f"ISSUE43_FAST2_FEEDBACK_1E14: {feedback_small.get('class')}")
        print(f"ISSUE43_FAST2_FEEDBACK_1E14_RESIDUAL: {feedback_small.get('nonlinear_residual_summary')}")
    if feedback_large is not None:
        print(f"ISSUE43_FAST2_FEEDBACK_1E12: {feedback_large.get('class')}")
        print(f"ISSUE43_FAST2_FEEDBACK_1E12_RESIDUAL: {feedback_large.get('nonlinear_residual_summary')}")
    print(f"ISSUE43_FAST2_PRECLASS: {decision['class']}")
    print(f"ISSUE43_FAST2_REASON: {decision['reason']}")
    print(f"ISSUE43_FAST2_SUMMARY: {summary_path}")
