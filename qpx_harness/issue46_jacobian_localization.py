...
# Compatibility name for callers that previously caught the historical owner
# exception. The semantic owner itself does not import that owner.
AugmentedJacobianLocalizationError = Issue46JacobianLocalizationError

# Construction ownership is deliberately delegated to the thin recipe.
...
def audit_localization_structure(
    ...
    expected_flags = [
        flag for flag in po.get_flags(base_first_linear) if flag != "-snes_test_jacobian"
    ]
...
def _preflight_reports(
    ...
    p2_main = (
        _check_input(exe, prepared["main_dir"], prepared["root"] / "p2_c0_check_input.log")
        if p1_main
        else {}
    )
    p2_control = (
        _check_input(exe, prepared["control_dir"], prepared["root"] / "p2_control_check_input.log")
        if p1_control
        else {}
    )
    status = (
...
def run_runtime(qpx: str | None, results_root: str | None) -> int:
    ...
    control_analysis = analyze_framework_control_runtime(
        control_log.read_text(errors="replace"),
        returncode=control_run["returncode"],
    )
    control_worst = _finite_or_none(
        control_analysis.get("jacobian", {}).get("worst_relative_frobenius_error")
    )
    ...
    if control_analysis["status"] == "PASS":
        ...
        if not dofmap_path.is_file():
            c0_analysis = {
                "status": "HOLD",
                "class": "JACOBIAN_LOCALIZATION_INSUFFICIENT",
                "reason": f"expected DOFMap output is missing: {dofmap_path}",
            }
        else:
    ...
    if c0_analysis is not None:
        c0_worst = _finite_or_none(
            c0_analysis.get("jacobian", {}).get("worst_relative_frobenius_error")
        )
        print(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        Issue46JacobianLocalizationError,
