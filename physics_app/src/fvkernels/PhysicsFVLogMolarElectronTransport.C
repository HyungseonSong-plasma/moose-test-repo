...
PhysicsFVLogMolarElectronTimeDerivative::validParams()
{
  ...
  params.addClassDescription(
      "Conservative backward-Euler time derivative of electron molar concentration "
      "c_e = exp(log_e), with log_e the solved FV variable.");
...
PhysicsFVLogMolarElectronTimeDerivative::computeQpResidual()
{
  if (!_subproblem.isTransient())
    mooseError("PhysicsFVLogMolarElectronTimeDerivative requires a transient executioner.");
  if (_dt <= 0.0)
  ...
  // This object is intentionally qualified only with scheme=implicit-euler.
  // Use the exact discrete conservative-state difference rather than the
  // chain-rule approximation exp(log_c) * d(log_c)/dt.
...
PhysicsFVLogMolarElectronDiffusion::validParams()
{
  ...
  params.addParam<MooseEnum>(
      "coeff_interp_method",
      coeff_interp_method,
      "Face interpolation method for the diffusion coefficient.");
...
PhysicsFVLogMolarElectronDiffusion::computeQpResidual()
{
  ...
  if (_var.isInternalFace(*_face_info))
  {
    ...
    interpolate(
        _coeff_interp_method, coeff_face, coeff_elem, coeff_neighbor, *_face_info, true);
    ...
    const auto centered_face =
        makeFace(*_face_info,
                 ...
                 false,
                 &limiter_time);
    log_c_face = _var(centered_face, state);
  }
  else
  {
    const auto face = singleSidedFaceArg();
    coeff_face = _coeff(face, state);
    log_c_face = _var(face, state);
  }

  const ADReal dlogcdn = gradUDotNormal(state, _correct_skewness);
  return -coeff_face * exp(log_c_face) * dlogcdn;
}
...
PhysicsFVLogMolarElectrostaticDrift::validParams()
{
  ...
  params.addRequiredParam<MooseFunctorName>(
      "carrier", "Multiplicative carrier/conversion functor; use 1 for electron molar density.");
...
PhysicsFVLogMolarElectrostaticDrift::computeQpResidual()
{
  ...
  const ADReal log_c_face = _var(transported_face, state);
  return carrier_face * exp(log_c_face) * drift_normal;
}
