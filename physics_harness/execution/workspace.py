...
GENERATED_DIR_NAMES = {
    ".jitcache", "__pycache__", "checkpoint", "checkpoints", "result",
...
class ManifestStatus:
    ...
    @property
    def ok(self) -> bool:
        if not self.json_ok or self.test_type not in VALID_TEST_TYPES:
            return False
...
def is_generated_candidate(path: Path, root: Path) -> bool:
    ...
    if path.suffix.lower() in GENERATED_SUFFIXES:
        return True
    if "_out" in path.name.lower() and path.suffix.lower() in {".csv", ".e", ".exo"}:
        return True
    return bool(re.search(r"(?:^|/)[^/]*_cp(?:/|$)", rel.as_posix().lower()))
...
def validate_manifest(manifest: Path) -> ManifestStatus:
    ...
    try:
        ...
        return ManifestStatus(
            path=str(manifest), test_type=None, json_ok=False, input_name=None,
            input_state=None, checker=None, checker_resolves=None, prepare=None,
            prepare_resolves=None, error=str(exc),
        )
    ...
    if input_name:
        ...
        input_state = "missing"
    error = None if test_type in VALID_TEST_TYPES else f"invalid test type: {test_type!r}"
    return ManifestStatus(
        path=str(manifest), test_type=test_type, json_ok=True, input_name=input_name,
        input_state=input_state, checker=checker, checker_resolves=checker_resolves,
        prepare=prepare, prepare_resolves=prepare_resolves, error=error,
    )
...
def inventory_workspace(root: Path) -> WorkspaceInventory:
    ...
    return WorkspaceInventory(root=str(root), files=files, manifests=manifests)
...
def summarize_workspace(inventory: WorkspaceInventory) -> dict:
    ...
    return {
        ...
        "canonical_count": sum(1 for item in inventory.manifests if item.test_type == "canonical"),
        "diagnostic_count": sum(1 for item in inventory.manifests if item.test_type == "diagnostic"),
        "generated_candidate_count": sum(1 for item in inventory.files if item.generated_candidate),
...
def compare_workspaces(source: WorkspaceInventory, target: WorkspaceInventory) -> WorkspaceComparison:
    ...
    for item in source.files:
        ...
        identity_matches = (
            ...
            if item.manifest_identity else []
        )
        if hash_matches:
            relationship, matches = "EXACT_DUPLICATE", hash_matches
        elif rel_matches:
            relationship, matches = "SAME_RELATIVE_PATH_DIFFERENT_BYTES", rel_matches
        elif identity_matches:
            relationship, matches = "SAME_MANIFEST_IDENTITY_DIFFERENT_BYTES", identity_matches
        else:
            relationship, matches = "SOURCE_ONLY", []
        records.append(
            ComparisonRecord(
    ...
    return WorkspaceComparison(
        source_root=source.root, target_root=target.root, records=tuple(records)
    )
...
def inventory_cli(argv: list[str] | None = None) -> int:
    ...
    if args.json_out:
        ...
        print(f"JSON        {out}")
    return 1 if args.strict and summary["invalid_manifest_count"] else 0
...
def self_test() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (left / "Issue1_a" / "test.json").write_text(json.dumps({
            "name": "case_a", "type": "canonical", "input": "input.i", "checker": "check.py"
        }))
        ...
        (right / "Issue1_a" / "test.json").write_text(json.dumps({
            "name": "case_a", "type": "canonical", "input": "created.i", "prepare": "prepare.py"
        }))
        ...
        (left / "Issue2_b" / "test.json").write_text(json.dumps({
            "name": "case_b", "type": "diagnostic", "input": "left.i"
        }))
        (right / "Issue8_b" / "test.json").write_text(json.dumps({
            "name": "case_b", "type": "diagnostic", "input": "right.i"
        }))
