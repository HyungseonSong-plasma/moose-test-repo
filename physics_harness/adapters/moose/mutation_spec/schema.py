"""Strict, versioned Pydantic models for MOOSE MutationSpec v1."""
from __future__ import annotations

from typing import Any, Literal, Union

from .errors import MutationSpecDependencyError

try:
    from pydantic import BaseModel, Field
except ImportError as exc:  # pragma: no cover - environment contract
    raise MutationSpecDependencyError(
        "MOOSE MutationSpec v1 requires the 'pydantic' package"
    ) from exc
...
class MutationCaseSpec(StrictModel):
...
class MutationSpec(StrictModel):
    ...
    cases: tuple[MutationCaseSpec, ...] = Field(min_length=1)


def model_validate(payload: Any) -> MutationSpec:
    """Validate one payload on either supported Pydantic major API."""
    if _PYDANTIC_V2:
        return MutationSpec.model_validate(payload)
    return MutationSpec.parse_obj(payload)
