# ==============================================================================
# R15 EVR2 — real-qvt Q-1 migration integration (migration_correction_off)
#
...
[FunctorMaterials]
  ...
  [x_rho_w_O2_material]
    type = ADParsedFunctorMaterial
    property_name = x_rho_w_O2
    functor_names = 'rho_mat w_O2_constraint'
    functor_symbols = 'rho_m wf_m'
    expression = 'x*rho_m*wf_m'
    block = plasma
  []

  [x_rho_w_O2s_material]
    type = ADParsedFunctorMaterial
    property_name = x_rho_w_O2s
    functor_names = 'rho_mat w_O2s'
    functor_symbols = 'rho_m wf_m'
    expression = 'x*rho_m*wf_m'
    block = plasma
  ...
  [x_rho_w_O2p_material]
    type = ADParsedFunctorMaterial
    property_name = x_rho_w_O2p
    functor_names = 'rho_mat w_O2p'
    functor_symbols = 'rho_m wf_m'
    expression = 'x*rho_m*wf_m'
    block = plasma
  ...
  # COMSOL mixture-averaged ion mobility default (singly charged ions):
  #   mu_k,m = q_e * D_k,m / (k_B * T_ion), with T_ion = T_g here.
  # QPXFVElectrostaticDrift owns the signed charge number separately.
  [mobility_O2p_einstein]
    type = ADParsedFunctorMaterial
    property_name = mu_O2p
    functor_names = 'D_mix_O2p T_g'
    functor_symbols = 'dmix_i temp_i'
    expression = '1.602176634e-19*dmix_i/(1.380649e-23*temp_i)'
    block = plasma
  []
  [mobility_Om_einstein]
    type = ADParsedFunctorMaterial
    property_name = mu_Om
