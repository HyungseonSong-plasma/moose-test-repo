...
_LANES = {
    "child_accepted": {"pp_execute_on": "INITIAL TIMESTEP_END", "output_execute_on": "INITIAL TIMESTEP_END", "output": "issue236_diag_child_accepted_csv", "file_base": "issue236_diag_child_accepted"},
    "parent_after_transfer": {"pp_execute_on": "INITIAL TIMESTEP_BEGIN", "output_execute_on": "INITIAL TIMESTEP_BEGIN", "output": "issue236_diag_parent_after_transfer_csv", "file_base": "issue236_diag_parent_after_transfer"},
    "parent_nonlinear": {"pp_execute_on": "NONLINEAR", "output_execute_on": "NONLINEAR", "output": "issue236_diag_parent_nonlinear_csv", "file_base": "issue236_diag_parent_nonlinear"},
}
...
def _insert_extreme_pp(text: str, *, lane: str, field_key: str, functor: str, value_type: str) -> str:
    spec = _LANES[lane]
    name = f"issue236_diag_{lane}_{field_key}_{value_type}"
    return _insert_pp(text, name, f"""  [{name}]
...
def _insert_lane_output(text: str, *, lane: str) -> str:
    ...
    return base.mb.insert_child_block(text, "Outputs", f"""  [{spec['output']}]
    type = CSV
...
def _instrument_lane(text: str, *, lane: str, fields: dict[str, str]) -> str:
    for field_key, functor in fields.items():
        for value_type in ("min", "max"):
            text = _insert_extreme_pp(text, lane=lane, field_key=field_key, functor=functor, value_type=value_type)
    for suffix, pp_type in (("step", "NumTimeSteps"), ("failed", "NumFailedTimeSteps"), ("dt", "TimestepSize")):
...
def _add_snapshot_variables_and_transfer(text: str) -> str:
    ...
    for _key, (_source, target, initial) in _SNAPSHOTS.items():
        ...
        text = base.mb.insert_child_block(text, "AuxVariables", f"""  [{target}]
  ...
  []""")
    ...
    return base.mb.insert_child_block(text, "Transfers", f"""  [{_SNAPSHOT_TRANSFER}]
...
def _build_parent_input(production_text: str) -> str:
    ...
    return _instrument_lane(text, lane="parent_nonlinear", fields=fields)
...
def _lane_audit(text: str, *, lane: str, fields: dict[str, str]) -> dict[str, bool]:
    ...
    checks[f"diag_output:{lane}"] = (base.mb.has_block(text, output_path) and base.mp.unquote(base.mp.get_parameter(text, output_path, "type")) == "CSV" and base.mp.unquote(base.mp.get_parameter(text, output_path, "file_base")) == spec["file_base"] and base.mp.unquote(base.mp.get_parameter(text, output_path, "execute_on")) == spec["output_execute_on"] and "FAILED" not in (base.mp.unquote(base.mp.get_parameter(text, output_path, "execute_on")) or ""))
    expected = []
    for field_key in fields:
        for value_type in ("min", "max"):
            expected.append(f"issue236_diag_{lane}_{field_key}_{value_type}")
    expected.extend((f"issue236_diag_{lane}_step", f"issue236_diag_{lane}_failed", f"issue236_diag_{lane}_dt"))
    for name in expected:
        path = f"Postprocessors/{name}"
        checks[f"diag_pp:{name}"] = (base.mb.has_block(text, path) and base.mp.unquote(base.mp.get_parameter(text, path, "execute_on")) == spec["pp_execute_on"] and base.mp.words(base.mp.get_parameter(text, path, "outputs")) == [spec["output"]])
    return checks
...
def _schedule_audit(text: str) -> dict[str, bool]:
    ...
    return {"diag_multiapp_runs_at_timestep_begin": ma_exec == "TIMESTEP_BEGIN", "diag_operational_transfer_same_as_multiapp": operational_exec in (None, "", "SAME_AS_MULTIAPP", "TIMESTEP_BEGIN"), "diag_snapshot_transfer_at_timestep_begin": snapshot_exec == "TIMESTEP_BEGIN"}
...
def _audit_parent(text: str):
    ...
    operational = "Transfers/fast_from_electron"
    snapshot = f"Transfers/{_SNAPSHOT_TRANSFER}"
    result["checks"]["diag_operational_payload_unchanged"] = (base.mp.words(base.mp.get_parameter(text, operational, "source_variable")) == list(base.FAST_SOLVER_VARIABLES) and base.mp.words(base.mp.get_parameter(text, operational, "variable")) == list(base.FAST_SOLVER_VARIABLES))
    result["checks"]["diag_snapshot_payload_matches_operational_source"] = (base.mp.words(base.mp.get_parameter(text, snapshot, "source_variable")) == list(base.FAST_SOLVER_VARIABLES) and base.mp.words(base.mp.get_parameter(text, snapshot, "variable")) == [target for _source, target, _initial in _SNAPSHOTS.values()])
    return _finalize(result)


def _audit_child(text: str, *, dt_e: float):
    result = _nochem_audit_child(text, dt_e=dt_e)
    result["checks"].update(_lane_audit(text, lane="child_accepted", fields=_BASE_FIELDS))
    return _finalize(result)
...
def _self_test():
    ...
    checks["diag_schedule_unchanged"] = (math.isclose(meta["dt_e_s"], 1.0e-10, rel_tol=0.0, abs_tol=0.0) and math.isclose(meta["dt_h_s"], 1.0e-8, rel_tol=0.0, abs_tol=0.0) and meta["subcycles_per_heavy"] == 100)
    return _finalize(result)


def _find_diag_csv(case_dir: Path, file_base: str) -> Path | None:
    candidates = [path for path in case_dir.rglob("*.csv") if file_base in path.name and path.is_file()]
...
def _attempt_key(row: dict[str, float], lane: str) -> tuple[int, int, float, float] | None:
    names = (f"issue236_diag_{lane}_step", f"issue236_diag_{lane}_failed", f"issue236_diag_{lane}_dt", "time")
    if not all(name in row for name in names):
        return None
    return (int(round(row[names[0]])), int(round(row[names[1]])), row[names[2]], row[names[3]])
...
def _same_attempt(a: tuple[int, int, float, float] | None, b: tuple[int, int, float, float] | None) -> bool:
    if a is None or b is None or a[:2] != b[:2]:
        return False
    return math.isclose(a[2], b[2], rel_tol=0.0, abs_tol=1e-20) and math.isclose(a[3], b[3], rel_tol=0.0, abs_tol=1e-18)
...
def _compare_values(
    ...
    for label, (left_key, right_key) in pairs.items():
        ...
        result["fields"][label] = {"left": lv, "right": rv, "absolute_difference": abs(lv - rv), "relative_difference": abs(lv - rv) / scale}
    return result
...
def _within_parent_transfer_compare(row: dict[str, float] | None) -> dict[str, Any]:
    ...
    for state, snap in (("ne", "ne_snap"), ("nepsilon", "nepsilon_snap"), ("phi", "phi_snap")):
        for extreme in ("min", "max"):
            pairs[f"{state}_{extreme}"] = (f"issue236_diag_parent_after_transfer_{state}_{extreme}", f"issue236_diag_parent_after_transfer_{snap}_{extreme}")
    return _compare_values(row, row, pairs)


def _between_parent_lanes(begin: dict[str, float] | None, nonlinear: dict[str, float] | None) -> dict[str, Any]:
    pairs: dict[str, tuple[str, str]] = {}
    for field in (*_BASE_FIELDS.keys(), *_SNAPSHOTS.keys(), "heavy_ne_input"):
        for extreme in ("min", "max"):
            pairs[f"{field}_{extreme}"] = (f"issue236_diag_parent_after_transfer_{field}_{extreme}", f"issue236_diag_parent_nonlinear_{field}_{extreme}")
    return _compare_values(begin, nonlinear, pairs)


def _collect_transfer_diagnostics(case_dir: Path) -> dict[str, Any]:
    paths = {lane: _find_diag_csv(case_dir, spec["file_base"]) for lane, spec in _LANES.items()}
...
def _runtime_analysis(case_dir: Path, *, dt_e: float, returncode: int, timed_out: bool):
    result = _nochem_runtime_analysis(case_dir, dt_e=dt_e, returncode=returncode, timed_out=timed_out)
