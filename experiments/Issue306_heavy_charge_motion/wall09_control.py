#!/usr/bin/env python3
"""Issue #306 Sequence 09: Bohm-positive-ion timestep-ratio study with sheath-suppressed electron wall loss.

Physics held fixed:
  * electron chi_e = 100
  * right wall = O2+/O+ Bohm surface loss, O- thermal sticking + signed migration
...
Relative to Sequence 08, this changes only the positive-ion surface law from thermal sticking to Bohm while preserving the sheath-suppressed electron wall and the equal-time heavy-update cadence matrix.
"""
...
REPO = ROOT.parents[1]
GENERATED = ROOT / "generated_wall09"
RESULTS = ROOT / "results_wall09"
...
SPECS = tuple(
    {
        "name": f"bohm_sheath_ratio{ratio}",
        "mode": "comsol",
        "chi": CHI_E,
        "chi_h": CHI_E * ratio,
        ...
        json.dumps(
            {
                "issue": 306,
                "sequence": 9,
                "objective": "Bohm-positive-ion plus sheath-suppressed-electron-wall heavy timestep sensitivity at equal physical time",
                "chi_e": CHI_E,
                "ratios": list(RATIOS),
                ...
                "final_tau": FINAL_TAU,
                "wall_model": "O2+/O+ Bohm surface + migration; O- thermal sticking + migration; standard-MOOSE electron sheath suppression",
                "cases": built,
            },
            indent=2,
            sort_keys=True,
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{rel}/generated_wall09/{name}/input.i"
...
def p2() -> None:
    ...
    for name in CASE_NAMES:
        checks.append(
            f"cd /workspace/{rel}/generated_wall09/{name} && "
...
def run_case(case_name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/wall09_control.py --inner-run {case_name}"
    )
    result, code = analyze(case_name)
    (RESULTS / f"{case_name}_result.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print("ISSUE306_WALL09_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    if missing:
        raise RuntimeError(f"missing wall09 result(s): {missing}")
    ...
    if not valid:
        return {
            "issue": 306,
            "sequence": 9,
            "classification": "WALL09_INVALID_EVIDENCE",
            "evidence_valid": False,
            "cases": found,
        }

    ref = found["bohm_sheath_ratio2"]
    comparisons = {
        "ratio4_vs_ratio2": _comparison(ref, found["bohm_sheath_ratio4"]),
        "ratio8_vs_ratio2": _comparison(ref, found["bohm_sheath_ratio8"]),
        "ratio8_vs_ratio4": _comparison(
            found["bohm_sheath_ratio4"], found["bohm_sheath_ratio8"]
        ),
    }
    return {
        "issue": 306,
        "sequence": 9,
        "classification": "SHEATH_BOHM_POSITIVE_ION_HEAVY_DT_RATIO_EVIDENCE_COMPLETE",
        "evidence_valid": True,
        "chi_e": CHI_E,
        "ratios": list(RATIOS),
        "chi_h": [CHI_E * r for r in RATIOS],
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue306_wall09_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE306_WALL09_CLASSIFICATION:", summary["classification"])
