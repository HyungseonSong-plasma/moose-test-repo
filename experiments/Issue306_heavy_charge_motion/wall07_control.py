#!/usr/bin/env python3
"""Issue #306 Sequence 07: thermal-wall heavy-timestep ratio study.

Physics held fixed:
  * electron chi_e = 100
  * right wall = thermal charged-heavy loss + signed migration
  * electron wall = accepted right-only 1/2 particle and 5/6 energy closure
...
This isolates heavy-update cadence from physical integration time.
"""
...
REPO = ROOT.parents[1]
GENERATED = ROOT / "generated_wall07"
RESULTS = ROOT / "results_wall07"
        ...
        json.dumps(
            {
                "issue": 306,
                "sequence": 7,
                "objective": "thermal-wall heavy timestep sensitivity at equal physical time",
                "chi_e": CHI_E,
                "ratios": list(RATIOS),
                ...
                "final_tau": FINAL_TAU,
                "wall_model": "thermal charged-heavy wall loss + signed migration",
                "cases": built,
            },
            indent=2,
            sort_keys=True,
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{rel}/generated_wall07/{name}/input.i"
...
def p2() -> None:
    ...
    for name in CASE_NAMES:
        checks.append(
            f"cd /workspace/{rel}/generated_wall07/{name} && "
...
def run_case(case_name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/wall07_control.py --inner-run {case_name}"
    )
    result, code = analyze(case_name)
    (RESULTS / f"{case_name}_result.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print("ISSUE306_WALL07_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    if not valid:
        return {
            "issue": 306,
            "sequence": 7,
            "classification": "WALL07_INVALID_EVIDENCE",
            "evidence_valid": False,
            "cases": found,
        }

    ref = found["thermal_ratio2"]
    comparisons = {
        "ratio4_vs_ratio2": _comparison(ref, found["thermal_ratio4"]),
        "ratio8_vs_ratio2": _comparison(ref, found["thermal_ratio8"]),
        "ratio8_vs_ratio4": _comparison(
            found["thermal_ratio4"], found["thermal_ratio8"]
        ),
    }
    return {
        "issue": 306,
        "sequence": 7,
        "classification": "THERMAL_HEAVY_DT_RATIO_EVIDENCE_COMPLETE",
        "evidence_valid": True,
        "chi_e": CHI_E,
        "ratios": list(RATIOS),
        "chi_h": [CHI_E * r for r in RATIOS],
        ...
        "interpretation_guard": (
            "All cases use the same thermal wall physics and the same final physical "
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue306_wall07_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE306_WALL07_CLASSIFICATION:", summary["classification"])
