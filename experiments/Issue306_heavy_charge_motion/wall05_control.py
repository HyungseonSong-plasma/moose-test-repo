#!/usr/bin/env python3
"""Issue #306 Sequence 05: long heavy-relaxation convergence at fixed chi_e=20.
...
REPO = ROOT.parents[1]
GENERATED = ROOT / "generated_wall05"
RESULTS = ROOT / "results_wall05"
...
def build(clean: bool = True) -> list[dict[str, object]]:
    """Generate the 100/200/300-cycle fixed-chi matrix."""
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{ROOT.relative_to(REPO)}/generated_wall05/{name}/input.i"
...
def p2() -> None:
    ...
    for name in CASE_NAMES:
        checks.append(
            f"cd /workspace/{rel}/generated_wall05/{name} && "
            f"/workspace/physics_app/physics-opt --check-input -i input.i "
            f"> /workspace/{rel}/results_wall05/{name}_parent_p2.log 2>&1"
        )
        checks.append(
            f"cd /workspace/{rel}/generated_wall05/{name} && "
            f"/workspace/physics_app/physics-opt --check-input -i fast_sub.i "
            f"> /workspace/{rel}/results_wall05/{name}_fast_p2.log 2>&1"
        )
    script = (
...
def run_case(case_name: str) -> None:
    ...
    script = (
        ...
        f"python3 /workspace/{rel}/wall05_control.py --inner-run {case_name}"
    )
    base._docker(script)
    ...
    print("ISSUE306_WALL05_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    """Aggregate 100/200/300-cycle convergence evidence."""
    ...
    if missing:
        raise RuntimeError(f"missing wall05 result(s): {missing}")
    if not all(bool(found[name].get("evidence_valid")) for name in CASE_NAMES):
        return {
            "issue": 306,
            "sequence": 5,
            "classification": "WALL05_INVALID_EVIDENCE",
    ...
    d200_300 = abs(float(comparisons["cycles300_vs_200"]["phi_avg_change_V"]))
    return {
        "issue": 306,
        "sequence": 5,
        "classification": "ULTRALONG_HEAVY_RELAXATION_EVIDENCE_COMPLETE",
        "evidence_valid": True,
        "chi_e": CHI_E,
        "chi_h": CHI_H,
        "heavy_cycles": list(CYCLES),
        ...
        "potential_increment_ratio_300_200_over_200_100": (
            d200_300 / max(d100_200, 1.0e-30)
        ),
        "interpretation_guard": (
            "A decreasing 200->300 potential/profile increment relative to 100->200 "
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue306_wall05_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE306_WALL05_CLASSIFICATION:", summary["classification"])
        print(json.dumps(summary, indent=2, sort_keys=True))
