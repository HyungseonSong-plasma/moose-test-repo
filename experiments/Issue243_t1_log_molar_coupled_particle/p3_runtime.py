...
def _metrics(case_dir: Path, input_text: str, meta: dict[str, Any]) -> dict[str, Any]:
    ...
    electron_accumulation = (w45.s5r._num(final, "n_e_inventory") - w45.s5r._num(initial, "n_e_inventory")) / dt
    ...
    electron_defect = w45._rel_defect(electron_accumulation, expected_electron_rate, volumetric_electron_rate, see_rate, primary_rate)
    ...
    energy_accum_norm_rate = (w45.s5r._num(final, "s5r_n_epsilon_inventory") - w45.s5r._num(initial, "s5r_n_epsilon_inventory")) / dt
    ...
    expected_energy_norm_rate = volumetric_energy_norm_rate - primary_power / energy_scale + see_power / energy_scale
    energy_defect = w45._rel_defect(energy_accum_norm_rate, expected_energy_norm_rate, volumetric_energy_norm_rate, primary_power / energy_scale, see_power / energy_scale)
    ...
    for species, cfg in w45.combined.CHARGED.items():
        ion_rates[species] = AVOGADRO * float(wall["charged"][species]["total_mass_rate_kg_s"]) / float(cfg["molar_mass"])
    ...
    expected_charge_delta = positive_ion_charge_delta + negative_ion_charge_delta + primary_electron_charge_delta + see_electron_charge_delta
    measured_charge_delta = w45.s5r._num(final, "r31_charge_integral") - w45.s5r._num(initial, "r31_charge_integral")
    charge_scale = max(abs(positive_ion_charge_delta) + abs(negative_ion_charge_delta) + abs(primary_electron_charge_delta) + abs(see_electron_charge_delta), 1.0e-300)
    ...
    composition_error = max(abs(w45.s5r._num(final, "sum_w_min") - 1.0), abs(w45.s5r._num(final, "sum_w_max") - 1.0))
    ...
    return {
        ...
        "charge": {"ion_incident_particle_rate_s-1": ion_rates, "expected_delta_C": expected_charge_delta, "measured_delta_C": measured_charge_delta, "relative_defect_over_boundary_current_scale": charge_defect},
...
def _evaluate(meta: dict[str, Any], metrics: dict[str, Any], input_text: str) -> dict[str, Any]:
    gates = {
        ...
        "G09_convergence_positivity": metrics["n_e_min_m3"] >= 0.0 and metrics["n_epsilon_min"] > 0.0 and metrics["mean_energy_avg_eV"] > 0.0 and metrics["composition_max_abs_error"] <= w45.COMPOSITION_ABS_TOL,
