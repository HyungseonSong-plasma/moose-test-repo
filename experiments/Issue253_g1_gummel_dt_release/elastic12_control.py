...
REPO = ROOT.parents[1]
GENERATED = ROOT / "generated_elastic12"
RESULTS = ROOT / "results_elastic12"
...
FINAL_TAU = 100.0
CASES = (
    {"name": "elastic_chi1", "chi": 1.0, "steps": 100, "fp_max": 100},
    {"name": "elastic_chi10", "chi": 10.0, "steps": 10, "fp_max": 300},
    {"name": "elastic_chi20", "chi": 20.0, "steps": 5, "fp_max": 600},
)
...
def build(clean: bool = True) -> list[dict[str, object]]:
    template = (ROOT / "elastic12_template.i").read_text(encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "sequence": 12,
                "objective": (
                    "Equal-time solved electron-energy + Joule + O2-elastic timestep sweep "
                    "for chi=1,10,20 at T=100 initial dielectric-relaxation times"
                ),
                "equal_final_time": True,
                "total_time_tau_epsilon_initial": FINAL_TAU,
                "energy_equation": "ON",
                "joule_heating": "ON",
                "elastic_collision": "ON",
...
def static_contract() -> dict[str, object]:
    cases = build()
    expected = {
        "elastic_chi1": (1.0, 100, 100),
        "elastic_chi10": (10.0, 10, 300),
        "elastic_chi20": (20.0, 5, 600),
    }
    ...
    for name, (chi, steps, fp_max) in expected.items():
        ...
        assert p["elastic_collision"] is True
...
def p0() -> None:
    print("ISSUE253_G2_ELASTIC12_P0: PASS")
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{ROOT.relative_to(REPO)}/generated_elastic12/{name}/input.i"
    ...
    _docker(script)
    print("ISSUE253_G2_ELASTIC12_P1: PASS")
...
def p2() -> None:
    if not GENERATED.exists():
        build()
    checks = "; ".join(
        f"cd /workspace/{ROOT.relative_to(REPO)}/generated_elastic12/{name} && "
        "/workspace/physics_app/physics-opt --check-input -i input.i "
        f"> /workspace/{ROOT.relative_to(REPO)}/results_elastic12/{name}_p2.log 2>&1"
        for name in CASE_NAMES
    )
    script = (
        ...
        "export METHOD=opt; "
        f"mkdir -p /workspace/{ROOT.relative_to(REPO)}/results_elastic12; "
        "make -C /workspace/physics_app -j2; test -x /workspace/physics_app/physics-opt; "
        + checks
    )
    _docker(script)
    print("ISSUE253_G2_ELASTIC12_P2: PASS")
...
def analyze_case(case_name: str) -> tuple[dict[str, object], int]:
    ...
    cumulative_elastic = sum(
        max(_f(row, "elastic_loss_applied_W_m2"), 0.0) * float(p["dt_s"])
        for row in rows[1:]
    )
    initial_energy = _f(rows[0], "energy_inventory_J_m2")
    final_energy = _f(final, "energy_inventory_J_m2")
    inferred_joule = (
        final_energy - initial_energy + cumulative_wall + cumulative_elastic
    )
...
def run_case(case_name: str) -> None:
    ...
    print("ISSUE253_G2_ELASTIC12_CASE:", case_name, result["classification"])
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    if missing:
        raise RuntimeError(f"missing elastic12 result(s): {missing}")

    if not all(bool(found[name].get("evidence_valid")) for name in CASE_NAMES):
        return {
            "issue": 253,
            "sequence": 12,
            "classification": "ELASTIC12_INVALID_EVIDENCE",
            "evidence_valid": False,
            "cases": found,
        }

    anchor = found["elastic_chi1"]
    anchor_profile = anchor["final_profile"]
    comparisons: dict[str, object] = {}
    for name in ("elastic_chi10", "elastic_chi20"):
        item = found[name]
        ...
        comparisons[name + "_vs_chi1"] = {
            ...
            "energy_inventory_relative_delta": (
                float(item["final_energy_inventory_J_m2"])
                / float(anchor["final_energy_inventory_J_m2"]) - 1.0
            ),
            "wall_power_relative_delta": (
                float(item["final_wall_energy_power_W_m2"])
                / float(anchor["final_wall_energy_power_W_m2"]) - 1.0
            ),
            "elastic_power_relative_delta": (
                float(item["final_elastic_loss_applied_W_m2"])
                / max(float(anchor["final_elastic_loss_applied_W_m2"]), 1.0e-300) - 1.0
            ),
            "cumulative_elastic_loss_relative_delta": (
                float(item["cumulative_elastic_energy_loss_J_m2"])
                / max(float(anchor["cumulative_elastic_energy_loss_J_m2"]), 1.0e-300) - 1.0
            ),
            "inferred_joule_input_relative_delta": (
                float(item["cumulative_inferred_joule_input_J_m2"])
                / max(abs(float(anchor["cumulative_inferred_joule_input_J_m2"])), 1.0e-300) - 1.0
            ),
            "fp_solve_ratio": (
            ...
            "wall_clock_ratio": (
                ...
                / max(float(anchor["elapsed_seconds"]), 1.0e-30)
            ),
        }
    ...
    return {
        "issue": 253,
        "sequence": 12,
        "classification": "ENERGY_JOULE_ELASTIC_CHI_1_10_20_COMPLETE",
        "evidence_valid": True,
        "equal_final_time": True,
        "total_time_tau_epsilon_initial": FINAL_TAU,
        "anchor": "elastic_chi1",
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        out = RESULTS / "elastic12_summary.json"
        out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print("ISSUE253_G2_ELASTIC12_CLASSIFICATION:", summary["classification"])
