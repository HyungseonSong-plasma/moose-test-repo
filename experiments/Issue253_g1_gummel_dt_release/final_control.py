...
REPO = ROOT.parents[1]
GENERATED = ROOT / "generated_final"
        ...
        json.dumps(
            {
                ...
                "sequence": 4,
                "objective": (
                    "fixed-total-time final-state and runtime comparison: "
                    "chi=0.1 reference vs relaxed chi=10 and chi=100"
                ),
...
def static_contract() -> dict[str, object]:
    ...
    assert c100["steps"] * c100["fp_max"] == TARGET_TOTAL_COUPLING_BUDGET
...
def analyze_result() -> tuple[dict[str, object], int]:
    ...
    summary: dict[str, object] = {
        "issue": 253,
        "sequence": 4,
        "comparison": "fixed T=200 tau: chi=0.1 reference vs relaxed chi=10 and chi=100",
        "total_time_tau_epsilon": FINAL_TAU,
    ...
    for name in ("relaxed_chi10", "relaxed_chi100"):
    ...
    if all(c == "FINAL_STATE_REFERENCE_RECOVERED" for c in classes):
        summary["classification"] = "CHI10_AND_CHI100_FINAL_STATE_SUPPORTED"
    elif any(c == "FINAL_STATE_REFERENCE_RECOVERED" for c in classes):
        summary["classification"] = "PARTIAL_FINAL_STATE_SUPPORT"
    else:
        summary["classification"] = "NO_FINAL_STATE_SUPPORT_AT_CHI10_OR_CHI100"
...
def p0() -> None:
    ...
    print("ISSUE253_G1_FINAL_P0: PASS")
    ...
    script = (
        ...
        + "; ".join(
            (
                ...
                f"generated_final/{name}/input.i"
            )
            for name in ("ref_chi0p1", "relaxed_chi10", "relaxed_chi100")
        )
    )
    _docker(script)
    print("ISSUE253_G1_FINAL_P1: PASS")
...
def p2() -> None:
    ...
    script = (
        ...
        "export METHOD=opt; "
        "mkdir -p /workspace/experiments/Issue253_g1_gummel_dt_release/results_final; "
        "make -C /workspace/physics_app -j2; "
        "test -x /workspace/physics_app/physics-opt; "
        + checks
    )
    _docker(script)
    print("ISSUE253_G1_FINAL_P2: PASS")
...
def p3() -> None:
    ...
    script = (
        ...
        + "; ".join(
            (
                ...
                f"final_control.py --inner-run {name}"
            )
            for name in ("ref_chi0p1", "relaxed_chi10", "relaxed_chi100")
        )
    )
    ...
    RESULTS.mkdir(parents=True, exist_ok=True)
    out = RESULTS / "final_analysis.json"
    out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print("ISSUE253_G1_FINAL_CLASSIFICATION:", summary["classification"])
    print(json.dumps(summary, indent=2, sort_keys=True))
    if rc != 0:
        raise SystemExit(rc)
    print("ISSUE253_G1_FINAL_P3: EVIDENCE_READY")
