...
REPO = ROOT.parents[1]
GENERATED = ROOT / "generated_horizon11"
RESULTS = ROOT / "results_horizon11"
...
CASES = (
    {"name": "chi100_horizon_1chi", "horizon_chi": 1.0, "steps": 1, "fp_max": 3000},
    {"name": "chi100_horizon_10chi", "horizon_chi": 10.0, "steps": 10, "fp_max": 3000},
    {"name": "chi100_horizon_100chi", "horizon_chi": 100.0, "steps": 100, "fp_max": 3000},
)
...
def _params(spec: dict[str, object]) -> dict[str, object]:
    ...
    horizon_chi = float(spec["horizon_chi"])
    end_time = steps * DT
    if not math.isclose(horizon_chi, float(steps), rel_tol=0.0, abs_tol=0.0):
        raise RuntimeError(f"{spec['name']}: horizon_chi must equal physical step count at fixed chi")
    return {
        "name": str(spec["name"]),
        "chi": CHI,
        "horizon_chi": horizon_chi,
        "tau_epsilon_initial_s": tau,
        "dt_s": DT,
        "end_time_s": end_time,
        "end_time_tau_epsilon_initial": CHI * horizon_chi,
        "steps": steps,
        "fp_min": 2,
...
def build(clean: bool = True) -> list[dict[str, object]]:
    template = (ROOT / "horizon11_template.i").read_text(encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "sequence": 11,
                "objective": (
                    "Fixed-chi solved electron-energy + Joule-heating horizon sweep "
                    "at chi=100 for total times 1chi, 10chi, and 100chi"
                ),
                "fixed_chi": CHI,
                "fixed_dt_s": DT,
                "horizons_chi": [1.0, 10.0, 100.0],
                "horizons_tau_epsilon_initial": [100.0, 1000.0, 10000.0],
                "energy_equation": "ON",
                "joule_heating": "ON",
...
def static_contract() -> dict[str, object]:
    cases = build()
    expected = {
        "chi100_horizon_1chi": (1.0, 1, 3000),
        "chi100_horizon_10chi": (10.0, 10, 3000),
        "chi100_horizon_100chi": (100.0, 100, 3000),
    }
    by = {str(x["name"]): x for x in cases}
    assert set(by) == set(expected)
    for name, (horizon_chi, steps, fp_max) in expected.items():
        p = by[name]
        assert p["chi"] == CHI
        assert p["horizon_chi"] == horizon_chi
        ...
        assert math.isclose(float(p["end_time_s"]) / float(p["dt_s"]), horizon_chi, rel_tol=1e-14)
...
def p0() -> None:
    print("ISSUE253_G2_HORIZON11_P0: PASS")
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{ROOT.relative_to(REPO)}/generated_horizon11/{name}/input.i"
    ...
    _docker(script)
    print("ISSUE253_G2_HORIZON11_P1: PASS")
...
def p2() -> None:
    if not GENERATED.exists():
        build()
    checks = "; ".join(
        f"cd /workspace/{ROOT.relative_to(REPO)}/generated_horizon11/{name} && "
        "/workspace/physics_app/physics-opt --check-input -i input.i "
        f"> /workspace/{ROOT.relative_to(REPO)}/results_horizon11/{name}_p2.log 2>&1"
        for name in CASE_NAMES
    )
    script = (
        ...
        "export METHOD=opt; "
        f"mkdir -p /workspace/{ROOT.relative_to(REPO)}/results_horizon11; "
        "make -C /workspace/physics_app -j2; test -x /workspace/physics_app/physics-opt; "
        + checks
    )
    _docker(script)
    print("ISSUE253_G2_HORIZON11_P2: PASS")
...
def run_case(case_name: str) -> None:
    ...
    print("ISSUE253_G2_HORIZON11_CASE:", case_name, result["classification"])
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    if missing:
        raise RuntimeError(f"missing horizon11 result(s): {missing}")

    if not all(bool(found[name].get("evidence_valid")) for name in CASE_NAMES):
        return {
            "issue": 253,
            "sequence": 11,
            "classification": "HORIZON11_INVALID_EVIDENCE",
            "evidence_valid": False,
            "fixed_chi": CHI,
            "cases": found,
        }

    names = ("chi100_horizon_1chi", "chi100_horizon_10chi", "chi100_horizon_100chi")
    evolution: dict[str, object] = {}
    for earlier, later in zip(names[:-1], names[1:]):
        a = found[earlier]
        b = found[later]
        evolution[f"{later}_vs_{earlier}"] = {
            "elapsed_time_ratio": float(b["final_time_s"]) / float(a["final_time_s"]),
            "phi_min_delta_V": float(b["final_phi_min_V"]) - float(a["final_phi_min_V"]),
            "phi_max_delta_V": float(b["final_phi_max_V"]) - float(a["final_phi_max_V"]),
            "mean_energy_avg_delta_eV": float(b["final_mean_energy_avg_eV"]) - float(a["final_mean_energy_avg_eV"]),
            "n_e_min_relative_delta": float(b["final_n_e_min_m3"]) / float(a["final_n_e_min_m3"]) - 1.0,
            "wall_power_relative_delta": float(b["final_wall_energy_power_W_m2"]) / float(a["final_wall_energy_power_W_m2"]) - 1.0,
            "energy_inventory_relative_delta": float(b["final_energy_inventory_J_m2"]) / float(a["final_energy_inventory_J_m2"]) - 1.0,
            "fixed_point_iterations_delta": int(b["total_fixed_point_iterations"]) - int(a["total_fixed_point_iterations"]),
        }

    return {
        "issue": 253,
        "sequence": 11,
        "classification": "CHI100_HORIZON_1_10_100_COMPLETE",
        "evidence_valid": True,
        "fixed_chi": CHI,
        "fixed_dt_s": DT,
        "horizons_chi": [1.0, 10.0, 100.0],
        ...
        "evolution": evolution,
        "scientific_scope": (
            "time-horizon evolution at fixed chi=100; this is not an equal-final-time "
            "accuracy comparison across different chi values"
        ),
    }
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        out = RESULTS / "horizon11_summary.json"
        out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print("ISSUE253_G2_HORIZON11_CLASSIFICATION:", summary["classification"])
