...
def _instrument_source(text: str) -> tuple[str, dict[str, object]]:
    ...
    mandatory = (
        "PhysicsThermalDiffusionMaterial::evaluate",
        "_D_T_names",
        "_kT_names",
        "_D_mix_names",
    )
    ...
    try:
        body = cpp.function_body("PhysicsThermalDiffusionMaterial::evaluate")
...
Result
PhysicsThermalDiffusionMaterial::evaluate(const int r, const int state) const
{
  auto nDij = foo();
  for (int i = 0; i < 7; ++i)
  {
