"""Issue #315 Sequence 03: fixed-point relaxation-factor optimization.

Physics and time integration are frozen to the accepted Issue #306 Sequence 08 wall08 ratio4 baseline.
...
For chi_e=100 these are approximately 0.0247525, 0.0297030,
0.0346535, 0.0396040, and 0.0445545.
"""
...
GENERATED = ROOT / "generated_relaxation03"
RESULTS = ROOT / "results_relaxation03"
...
def p0() -> None:
    print("ISSUE315_ALPHA03_P0: PASS")
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{rel}/generated_relaxation03/{name}/input.i"
    ...
    print("ISSUE315_ALPHA03_P1: PASS")
...
def p2() -> None:
    ...
    for name in CASE_NAMES:
        checks.append(
            f"cd /workspace/{rel}/generated_relaxation03/{name} && "
    ...
    print("ISSUE315_ALPHA03_P2: PASS")
...
def run_case(case_name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/control_seq03.py --inner-run {case_name}"
    )
    result, code = analyze(case_name)
    (RESULTS / f"{case_name}_result.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print("ISSUE315_ALPHA03_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    return {
        "issue": 315,
        "sequence": 3,
        "classification": "ALPHA_REFINEMENT_COMPLETE" if complete else "ALPHA_REFINEMENT_PARTIAL",
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue315_alpha03_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE315_ALPHA03_CLASSIFICATION:", summary["classification"])
