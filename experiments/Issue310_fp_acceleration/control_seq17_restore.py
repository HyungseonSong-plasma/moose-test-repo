"""Issue #310 Generation 17: restore-semantics and stopping-tolerance discriminator.

Keep the measured row-sum-preserving +/-5 electron-Jacobian correction fixed,
then vary only:
  * TransientMultiApp restoration policy for the Poisson child; and
  * outer Picard relaxation / fixed-point relative tolerance.

The correction always vanishes for phi == phi_anchor.  These cases test whether
the remaining short-horizon parity offset comes from no_restore semantics or
from terminating the accelerated fixed point at the qualified 1e-8 tolerance.
"""
...
base = wall08.base
GENERATED = ROOT / "generated_fp17_restore"
RESULTS = ROOT / "results_fp17_restore"
...
SPECS = (
    ...
    {
        "name": "band5_restore_a020_tol1e8",
        "bandwidth": 5,
        "relaxation_factor": 0.20,
    ...
    {
        "name": "band5_restore_a100_tol1e8",
        "bandwidth": 5,
        "relaxation_factor": 1.00,
        "restore_mode": "restore",
        "fixed_point_rel_tol": 1.0e-8,
    ...
    {
        "name": "band5_nr_a100_tol1e10",
        "bandwidth": 5,
        "relaxation_factor": 1.00,
        "restore_mode": "no_restore",
        "fixed_point_rel_tol": 1.0e-10,
    },
    {
        "name": "band5_restore_a100_tol1e10",
        "bandwidth": 5,
        "relaxation_factor": 1.00,
        "restore_mode": "restore",
        "fixed_point_rel_tol": 1.0e-10,
    ...
    {
        "name": "band5_nr_a100_tol1e12",
        "bandwidth": 5,
        "relaxation_factor": 1.00,
        "restore_mode": "no_restore",
        "fixed_point_rel_tol": 1.0e-12,
    },
)
...
def _spec(raw: dict[str, object]) -> dict[str, object]:
    return {
        **raw,
        ...
        "restore_mode": str(raw["restore_mode"]),
        "fixed_point_rel_tol": float(raw["fixed_point_rel_tol"]),
    }
...
def _timeaware(raw: dict[str, object], p: dict[str, object]) -> tuple[str, str, str]:
    ...
    restore_mode = str(raw["restore_mode"])
    if restore_mode == "restore":
        anchor = "    no_restore = true\n"
        if fast.count(anchor) != 1:
            raise RuntimeError("TransientMultiApp no_restore anchor changed")
        fast = fast.replace(anchor, "    no_restore = false\n", 1)
    elif restore_mode != "no_restore":
        raise RuntimeError(f"unknown restore_mode: {restore_mode}")

    tol_anchor = "  fixed_point_rel_tol = 1.0e-8\n"
    if fast.count(tol_anchor) != 1:
        raise RuntimeError("fixed-point relative tolerance anchor changed")
    fast = fast.replace(
        tol_anchor,
        f"  fixed_point_rel_tol = {float(raw['fixed_point_rel_tol']):.17g}\n",
        1,
    )
...
def p0() -> None:
    ...
    for raw in SPECS[1:]:
        ...
        assert expected_tol in fast
        if str(raw["restore_mode"]) == "restore":
            assert "no_restore = false" in fast
        else:
            assert "no_restore = true" in fast
    print("ISSUE310_GEN17_RESTORE_P0: PASS")
    print(json.dumps({
        "band5": metrics,
        "cases": [
            {
                "name": x["name"],
                "alpha": x["relaxation_factor"],
                "restore_mode": x["restore_mode"],
                "fixed_point_rel_tol": x["fixed_point_rel_tol"],
            }
            for x in SPECS
        ],
    }, sort_keys=True))
...
def p1() -> None:
    ...
    cmds = "; ".join(
        f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp17_restore/{n}/input.i"
        for n in CASE_NAMES
    )
    base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; " + cmds)
    print("ISSUE310_GEN17_RESTORE_P1: PASS")
...
def p2() -> None:
    ...
    for n in CASE_NAMES:
        for f in ("input.i", "fast_sub.i", "poisson_sub.i"):
            checks.append(
                f"cd /workspace/{rel}/generated_fp17_restore/{n} && "
    ...
    print("ISSUE310_GEN17_RESTORE_P2: PASS")
...
def run_case(name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/control_seq17_restore.py --inner-run {name}; "
        f"chmod -R a+rwX /workspace/{rel}/results_fp17_restore /workspace/{rel}/generated_fp17_restore"
    )
    ...
    result.update(
        ...
        gen13_basis_run=GEN13_RUN,
        outer_relaxation_factor=float(raw["relaxation_factor"]),
        restore_mode=str(raw["restore_mode"]),
        fixed_point_rel_tol=float(raw["fixed_point_rel_tol"]),
    ...
    print("ISSUE310_GEN17_RESTORE_CASE:", name, result["classification"])
    if code:
...
def aggregate() -> None:
    ...
    summary = {
        ...
        "sequence": 17,
        "classification": (
            "GEN17_RESTORE_EVIDENCE_COMPLETE"
            if not missing else "GEN17_RESTORE_EVIDENCE_PARTIAL"
        ),
    ...
    RESULTS.mkdir(parents=True, exist_ok=True)
    out = RESULTS / "issue310_gen17_restore_summary.json"
    out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print("ISSUE310_GEN17_RESTORE_AGGREGATE:", summary["classification"])
