"""Issue #310 Gen27: native MOOSE PerfGraph timing discriminator.

Runs the frozen qualified 20 ns Steffensen case and requests MOOSE's native
performance log from the executable. No scientific input, convergence,
timestep, solver algorithm, or output configuration is changed.
"""
...
GENERATED = ROOT / "generated_fp27_perfgraph"
RESULTS = ROOT / "results_fp27_perfgraph"
MEDIUM_HEAVY_CYCLES = 50
MEDIUM_FINAL_TAU = g.FINAL_TAU * MEDIUM_HEAVY_CYCLES
SPECS = (
    {"name":"perfgraph_50hc","heavy_cycles":50,"suppress_fp_anchor_output":False,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    assert SPECS[0]["heavy_cycles"] == 50
    ...
    print("ISSUE310_GEN27_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))

def p1():
    build(); rel=ROOT.relative_to(REPO)
    cmds="; ".join(f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp27_perfgraph/{n}/input.i" for n in CASE_NAMES)
    g.base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; "+cmds)
    print("ISSUE310_GEN27_P1: PASS")

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp27_perfgraph/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def inner(name):
    ...
    with log.open("w",encoding="utf-8") as h:
        cp=subprocess.run([str(REPO/"physics_app"/"physics-opt"),"-i","input.i","--show-perf-graph"],cwd=case_dir,stdout=h,stderr=subprocess.STDOUT,check=False)
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq27_perfgraph.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp27_perfgraph /workspace/{rel}/generated_fp27_perfgraph")
    ...
    print("ISSUE310_GEN27_CASE",name,result["classification"])
...
def aggregate():
    ...
    summary={"issue":310,"sequence":27,"classification":"GEN27_COMPLETE" if not missing else "GEN27_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"Native MOOSE PerfGraph diagnostic only; exact Gen25 scientific/output configuration frozen. Full runtime log retained as evidence."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen27_perfgraph_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN27_AGGREGATE",summary["classification"])
