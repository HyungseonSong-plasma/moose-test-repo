"""Issue #310 Sequence 05: damped Secant / Steffensen fixed-point sweep.

Sequence 04 showed that full-step Secant and Steffensen produce the same first
nonlinear extrapolation and both overshoot the transport lookup range.
...
Physics, timestep, spatial discretization, nonlinear/fixed-point tolerances,
transfer ownership, and transformed variable potential_plasma are frozen.
"""
...
GENERATED = ROOT / "generated_fp05"
RESULTS = ROOT / "results_fp05"
        ...
        json.dumps(
            {
                "issue": 310,
                "sequence": 5,
                "objective": "resolve stable and efficient damping for native Secant/Steffensen at frozen Sequence08 physics",
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{rel}/generated_fp05/{name}/input.i"
...
def p2() -> None:
    ...
    for name in CASE_NAMES:
        checks.append(
            f"cd /workspace/{rel}/generated_fp05/{name} && "
...
def run_case(case_name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/control_seq05.py --inner-run {case_name}"
    )
    result, code = analyze(case_name)
    (RESULTS / f"{case_name}_result.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print("ISSUE310_FP05_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "sequence": 5,
        "classification": (
            "FP_DAMPING_SWEEP_COMPLETE" if complete else "FP_DAMPING_SWEEP_PARTIAL"
        ),
        ...
        "reference": {
            "case": "Sequence04 picard_2x",
            "fixed_point_algorithm": "picard",
            "relaxation_factor": 2.0 / (1.0 + CHI_E),
            "observed_fp_per_step": 596.5,
            "observed_total_fp_12_steps": 7158,
        },
        "target": {
        ...
        "interpretation_guard": (
            ...
            "Only native fixed-point algorithm and damping differ."
        ),
    }
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue310_fp05_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE310_FP05_CLASSIFICATION:", summary["classification"])
