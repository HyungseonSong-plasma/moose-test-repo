"""Issue #310 Gen21: native MOOSE secant/Steffensen acceleration on the qualified band5 map.

Gen20 qualified band5 alpha=0.45 at 25.25 FP/step against the corrected Gen19
delta-phi-aware root. This discriminator changes only the fixed-point iteration
algorithm applied to potential_from_poisson; physics, band5 operator, alpha,
no_restore, timestep hierarchy, and delta_phi_abs_tol=1e-6 remain frozen.
"""
...
GENERATED = ROOT / "generated_fp21_native"
RESULTS = ROOT / "results_fp21_native"
SPECS = (
    {"name":"band5_a045_secant_dphi1e6","bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"secant"},
    {"name":"band5_a045_steffensen_dphi1e6","bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    print("ISSUE310_GEN21_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))

def p1():
    build(); rel=ROOT.relative_to(REPO)
    cmds="; ".join(f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp21_native/{n}/input.i" for n in CASE_NAMES)
    g.base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; "+cmds)
    print("ISSUE310_GEN21_P1: PASS")

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp21_native/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def run_case(name):
    ...
    print("ISSUE310_GEN21_CASE",name,result["classification"])
...
def aggregate():
    ...
    summary={"issue":310,"sequence":21,"classification":"GEN21_COMPLETE" if not missing else "GEN21_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"Native MOOSE secant/Steffensen transforms potential_from_poisson only; band5/alpha/physics/no_restore/delta-phi criterion frozen; promotion requires corrected-root parity, stability, lower FP work, and wall time."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen21_native_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN21_AGGREGATE",summary["classification"])
