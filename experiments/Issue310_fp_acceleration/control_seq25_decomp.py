...
GENERATED = ROOT / "generated_fp25_decomp"
RESULTS = ROOT / "results_fp25_decomp"
MEDIUM_HEAVY_CYCLES = 50
MEDIUM_FINAL_TAU = g.FINAL_TAU * MEDIUM_HEAVY_CYCLES
SPECS = (
    {"name":"steffensen_50hc_decomp","heavy_cycles":50,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    print("ISSUE310_GEN25_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))

def p1():
    build(); rel=ROOT.relative_to(REPO)
    cmds="; ".join(f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp25_decomp/{n}/input.i" for n in CASE_NAMES)
    g.base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; "+cmds)
    print("ISSUE310_GEN25_P1: PASS")

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp25_decomp/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq25_decomp.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp25_decomp /workspace/{rel}/generated_fp25_decomp")
    ...
    print("ISSUE310_GEN25_CASE",name,result["classification"])
...
def aggregate():
    ...
    summary={"issue":310,"sequence":25,"classification":"GEN25_COMPLETE" if not missing else "GEN25_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"20 ns diagnostic rerun only; exact Steffensen band5 alpha=.45 physics/convergence/timestep configuration frozen. Added telemetry is post-run/process-level only."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen25_decomp_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN25_AGGREGATE",summary["classification"])
