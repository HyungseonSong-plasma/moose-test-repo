"""Issue #310 Gen24: horizon cost-growth profiler.

Runs the qualified band5 alpha=0.45 Steffensen configuration at 10, 25, and
50 heavy cycles (4, 10, 20 ns). Physics, timestep hierarchy, convergence,
band5 operator, alpha, and no_restore remain frozen. The discriminator
isolates horizon-dependent wall-time/FP cost growth without changing the
scientific model.
"""
...
GENERATED = ROOT / "generated_fp24_profile"
RESULTS = ROOT / "results_fp24_profile"
MEDIUM_HEAVY_CYCLES = 50
MEDIUM_FINAL_TAU = g.FINAL_TAU * MEDIUM_HEAVY_CYCLES
SPECS = (
    {"name":"steffensen_10hc","heavy_cycles":10,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
    {"name":"steffensen_25hc","heavy_cycles":25,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
    {"name":"steffensen_50hc","heavy_cycles":50,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    assert [x["heavy_cycles"] for x in SPECS] == [10,25,50]
    ...
    print("ISSUE310_GEN24_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))

def p1():
    build(); rel=ROOT.relative_to(REPO)
    cmds="; ".join(f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp24_profile/{n}/input.i" for n in CASE_NAMES)
    g.base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; "+cmds)
    print("ISSUE310_GEN24_P1: PASS")

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp24_profile/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq24_profile.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp24_profile /workspace/{rel}/generated_fp24_profile")
    result,code=g.seq08.analyze(name)
    result.update(sequence=24,profile_heavy_cycles=raw["heavy_cycles"],profile_final_tau=final_tau,banded_jacobian_width=5,outer_relaxation_factor=raw["relaxation_factor"],fixed_point_algorithm=raw["fp_algorithm"],custom_convergence=True,delta_phi_abs_tol=1e-6,gen19_reference_phi_avg_V=3.9792365725690155,**g._projection_metrics(5),**g._anchor_diagnostic(name))
    ...
    print("ISSUE310_GEN24_CASE",name,result["classification"])
...
def aggregate():
    ...
    summary={"issue":310,"sequence":24,"classification":"GEN24_COMPLETE" if not missing else "GEN24_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"Horizon-only profiler: same Steffensen band5 alpha=.45 physics and convergence at 10/25/50 heavy cycles; classify cost growth from measured artifacts only."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen24_profile_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN24_AGGREGATE",summary["classification"])
