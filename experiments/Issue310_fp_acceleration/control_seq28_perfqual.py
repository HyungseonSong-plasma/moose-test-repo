"""Issue #310 Gen28: qualify native MOOSE performance reporting before long profiling.

This generation first interrogates the built physics-opt help/configuration and
runs a short frozen scientific case using only a performance-report mechanism
that the executable itself advertises. P1 fails as infrastructure/harness if no
supported native timing mechanism can be established; no scientific conclusion
is permitted from that failure.
"""
...
GENERATED = ROOT / "generated_fp28_perfqual"
RESULTS = ROOT / "results_fp28_perfqual"
MEDIUM_HEAVY_CYCLES = 50
MEDIUM_FINAL_TAU = g.FINAL_TAU * MEDIUM_HEAVY_CYCLES
SPECS = (
    {"name":"perfqual_10hc","heavy_cycles":10,"suppress_fp_anchor_output":False,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    assert SPECS[0]["heavy_cycles"] == 10
    ...
    print("ISSUE310_GEN28_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))
...
def p1():
    ...
    print("ISSUE310_GEN28_P1: PASS", json.dumps(candidates))

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp28_perfqual/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def inner(name):
    ...
    with log.open("w",encoding="utf-8") as h:
        cp=subprocess.run([str(REPO/"physics_app"/"physics-opt"),"-i","input.i"],cwd=case_dir,stdout=h,stderr=subprocess.STDOUT,check=False)
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq28_perfqual.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp28_perfqual /workspace/{rel}/generated_fp28_perfqual")
    ...
    print("ISSUE310_GEN28_CASE",name,result["classification"])
...
def aggregate():
    ...
    summary={"issue":310,"sequence":28,"classification":"GEN28_COMPLETE" if not missing else "GEN28_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"Profiler qualification only. Short 10HC frozen scientific case; executable --help evidence retained. No unsupported command-line timing flag is injected."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen28_perfqual_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN28_AGGREGATE",summary["classification"])
