"""Issue #310 Generation 18: coupling-aware delta-phi convergence plus alpha sweep.

The Gen15/16 measured row-sum-preserving +/-5 electron-Jacobian correction and
no_restore=true architecture are frozen.  The default MOOSE fixed-point
residual criterion is augmented with an AND condition

    max_i |phi_i^(k) - phi_anchor_i^(k)| <= delta_phi_abs_tol.

First, alpha=0.2 compares the historical default stopping rule against
delta-phi tolerances 1e-6 and 1e-8 V.  Then delta_phi_abs_tol=1e-8 V is held
fixed while alpha is swept from 0.3 through 0.8 to locate the stability/performance
boundary without accepting a prematurely converged potential iterate.
"""
...
base = wall08.base
GENERATED = ROOT / "generated_fp18_deltaphi"
RESULTS = ROOT / "results_fp18_deltaphi"
...
SPECS = (
    ...
    {
        "name": "band5_a030_dphi1e8",
        "bandwidth": 5,
        "relaxation_factor": 0.30,
        "custom_convergence": True,
        "delta_phi_abs_tol": 1.0e-8,
    },
    {
        "name": "band5_a040_dphi1e8",
        "bandwidth": 5,
    ...
    {
        "name": "band5_a050_dphi1e8",
        "bandwidth": 5,
        "relaxation_factor": 0.50,
        "custom_convergence": True,
        "delta_phi_abs_tol": 1.0e-8,
    },
    {
        "name": "band5_a060_dphi1e8",
        "bandwidth": 5,
    ...
    {
        "name": "band5_a070_dphi1e8",
        "bandwidth": 5,
        "relaxation_factor": 0.70,
        "custom_convergence": True,
        "delta_phi_abs_tol": 1.0e-8,
    },
    {
        "name": "band5_a080_dphi1e8",
        "bandwidth": 5,
...
def _timeaware(raw: dict[str, object], p: dict[str, object]) -> tuple[str, str, str]:
    ...
    if "    no_restore = true\n" not in fast:
        raise RuntimeError("Gen18 requires qualified no_restore=true architecture")
...
def _apply_delta_phi_convergence(fast: str, tol: float) -> str:
    material_anchor = """  [electron_sheath_factor]
    type = ADParsedFunctorMaterial
"""
    if fast.count(material_anchor) != 1:
        raise RuntimeError("fast FunctorMaterials anchor changed")
    fast = fast.replace(
        material_anchor,
        """  [fp_delta_phi_abs]
    type = ADParsedFunctorMaterial
...
"""
    ...
    if fast.count(outputs_anchor) != 1:
        raise RuntimeError("Outputs anchor changed for convergence insertion")
...
def render(raw: dict[str, object], p: dict[str, object]) -> tuple[str, str, str]:
    ...
    if bandwidth > 0:
        ...
        if bool(raw["custom_convergence"]):
            fast = _apply_delta_phi_convergence(fast, float(raw["delta_phi_abs_tol"]))
    return parent, fast, poisson
        ...
        "cases": [
            {
                ...
                "custom_convergence": x["custom_convergence"],
                "delta_phi_abs_tol": x["delta_phi_abs_tol"],
            }
            for x in SPECS
        ],
    }, sort_keys=True))
...
def p1() -> None:
    ...
    cmds = "; ".join(
        f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp18_deltaphi/{n}/input.i"
        for n in CASE_NAMES
    )
    base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; " + cmds)
    print("ISSUE310_GEN18_DELTAPHI_P1: PASS")
...
def p2() -> None:
    ...
    for n in CASE_NAMES:
        for f in ("input.i", "fast_sub.i", "poisson_sub.i"):
            checks.append(
                f"cd /workspace/{rel}/generated_fp18_deltaphi/{n} && "
    ...
    print("ISSUE310_GEN18_DELTAPHI_P2: PASS")
...
def run_case(name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/control_seq18_deltaphi.py --inner-run {name}; "
        f"chmod -R a+rwX /workspace/{rel}/results_fp18_deltaphi /workspace/{rel}/generated_fp18_deltaphi"
    )
    ...
    print("ISSUE310_GEN18_DELTAPHI_CASE:", name, result["classification"])
    if code:
...
def aggregate() -> None:
    ...
    summary = {
        ...
        "sequence": 18,
        "classification": (
            "GEN18_DELTAPHI_EVIDENCE_COMPLETE"
            if not missing else "GEN18_DELTAPHI_EVIDENCE_PARTIAL"
        ),
    ...
    RESULTS.mkdir(parents=True, exist_ok=True)
    out = RESULTS / "issue310_gen18_deltaphi_summary.json"
    out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print("ISSUE310_GEN18_DELTAPHI_AGGREGATE:", summary["classification"])
