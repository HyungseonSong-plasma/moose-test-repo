...
The previous-potential anchor transfer is retained only so the same delta-phi convergence criterion can be evaluated. No `electron_response_beta`, `n_epsilon_frozen`, or `FVElectronResponseBandedCorrection` is present in the reference Poisson solve.
