"""Issue #310 Gen29: native MOOSE timing decomposition on the frozen 10HC case.

Gen28 qualified the executable's supported -t/--timing interface and reproduced
the 10-heavy-cycle scientific trajectory exactly. Gen29 changes only the runtime
reporting flag: it runs the same band5 alpha=0.45 Steffensen case with native
MOOSE timing enabled, retaining the full timing log for cost decomposition.
"""
...
GENERATED = ROOT / "generated_fp29_timing"
RESULTS = ROOT / "results_fp29_timing"
MEDIUM_HEAVY_CYCLES = 50
MEDIUM_FINAL_TAU = g.FINAL_TAU * MEDIUM_HEAVY_CYCLES
SPECS = (
    {"name":"timing_10hc","heavy_cycles":10,"suppress_fp_anchor_output":False,"bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    print("ISSUE310_GEN29_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))
...
def p1():
    ...
    print("ISSUE310_GEN29_P1: PASS", json.dumps(candidates))

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp29_timing/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq29_timing.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp29_timing /workspace/{rel}/generated_fp29_timing")
...
def aggregate():
    ...
    summary={"issue":310,"sequence":29,"classification":"GEN29_COMPLETE" if not missing else "GEN29_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"Native timing discriminator only. Exact Gen28 10HC scientific/convergence/output configuration frozen; the only runtime change is supported -t timing reporting. Full runtime log retained."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen29_timing_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN29_AGGREGATE",summary["classification"])
