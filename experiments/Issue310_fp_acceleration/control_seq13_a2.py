"""Issue #310 Generation 13 A2 small-gamma screened-Poisson discriminator.
...
base = wall08.base
GENERATED = ROOT / "generated_fp13_a2"
RESULTS = ROOT / "results_fp13_a2"
CHI_E = 100.0
CHI_H = 400.0
FINAL_TAU = 400.0
HEAVY_CYCLES = 1
...
SPECS = (
    ...
    {"name": "a2_gamma0025", "gamma": 0.025},
    {"name": "a2_gamma0050", "gamma": 0.05},
)
...
def _apply_screening(fast: str, poisson: str, gamma: float) -> tuple[str, str]:
  ...
  []
"""
    transfer_extra = transfer_anchor + """  [screen_mu_to_poisson]
    type = MultiAppCopyTransfer
    to_multi_app = poisson
...
"""
    ...
    functor_symbols = 'ne mu D'
    expression = '{gamma:.17g}*{E_OVER_EPS0:.17g}*ne*mu/max(D,1.0e-30)'
  []
...
def p0() -> None:
    ...
    print("ISSUE310_GEN13_A2_P0: PASS")
...
def p1() -> None:
    ...
    cmds = "; ".join(
        f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp13_a2/{n}/input.i"
        for n in CASE_NAMES
    )
    base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; " + cmds)
    print("ISSUE310_GEN13_A2_P1: PASS")
...
def p2() -> None:
    ...
    for n in CASE_NAMES:
        for f in ("input.i", "fast_sub.i", "poisson_sub.i"):
            checks.append(
                f"cd /workspace/{rel}/generated_fp13_a2/{n} && "
    ...
    print("ISSUE310_GEN13_A2_P2: PASS")
...
def run_case(name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/control_seq13_a2.py --inner-run {name}; "
        f"chmod -R a+rwX /workspace/{rel}/results_fp13_a2 /workspace/{rel}/generated_fp13_a2/{name}"
    )
    ...
    print("ISSUE310_GEN13_A2_CASE:", name, result.get("classification"))
    if code:
...
def aggregate() -> None:
    ...
    summary = {
        ...
        "sequence": 13,
        "lane": MODE,
        "classification": "GEN13_A2_EVIDENCE_COMPLETE" if not missing else "GEN13_A2_EVIDENCE_PARTIAL",
    ...
    RESULTS.mkdir(parents=True, exist_ok=True)
    out = RESULTS / "issue310_gen13_a2_summary.json"
    out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print("ISSUE310_GEN13_A2_AGGREGATE:", summary["classification"])
