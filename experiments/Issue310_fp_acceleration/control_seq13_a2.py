...
def p2() -> None:
    ...
    for n in CASE_NAMES:
        for f in ("input.i", "fast_sub.i", "poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp13_a2/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
    base._docker(
...
def run_case(name: str) -> None:
    ...
    base._docker(
        ...
        f"chmod -R a+rwX /workspace/{rel}/results_fp13_a2 /workspace/{rel}/generated_fp13_a2"
    )
...
def aggregate() -> None:
    ...
    RESULTS.mkdir(parents=True, exist_ok=True)
    out = RESULTS / f"issue310_gen13_a2_summary.json"
