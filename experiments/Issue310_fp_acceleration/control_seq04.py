"""Issue #310 Sequence 04: native fixed-point acceleration discriminator.

Physics and timestep are frozen to the accepted Issue #306 Sequence 08 ratio4 model.
The benchmark is intentionally short but preserves the heavy/electron cadence:
...
Cases:
  picard_2x       : Picard, subapp relaxation = 2/101 (qualified reference)
  secant_full     : Secant, full algorithm update (relaxation = 1)
  steffensen_full : Steffensen, full algorithm update (relaxation = 1)

The transformed state remains the existing FullSolveMultiApp subapp variable
potential_plasma. No plasma physics, spatial discretization, nonlinear
tolerances, fixed-point tolerances, or transfer ownership is changed.
"""
...
GENERATED = ROOT / "generated_fp04"
RESULTS = ROOT / "results_fp04"
...
ALGORITHM_CASES = (
    ...
    ("secant_full", "secant", 1.0),
    ("steffensen_full", "steffensen", 1.0),
)
        ...
        json.dumps(
            {
                "issue": 310,
                "sequence": 4,
                "objective": "benchmark native Picard/Secant/Steffensen fixed-point work at frozen Sequence08 physics",
...
def run_case(case_name: str) -> None:
    ...
    print("ISSUE310_FP04_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    baseline = found.get("picard_2x")
    comparisons: dict[str, object] = {}
    if baseline is not None:
        for name in ("secant_full", "steffensen_full"):
            if name in found:
                comparisons[f"{name}_vs_picard_2x"] = _comparison(baseline, found[name])

    complete = not missing
    return {
        "issue": 310,
        "sequence": 4,
        "classification": "FP_ACCELERATION_BENCHMARK_COMPLETE" if complete else "FP_ACCELERATION_BENCHMARK_PARTIAL",
        ...
        "interpretation_guard": (
            ...
            "Only the fixed-point algorithm and its algorithm damping differ."
        ),
    }
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue310_fp04_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE310_FP04_CLASSIFICATION:", summary["classification"])
