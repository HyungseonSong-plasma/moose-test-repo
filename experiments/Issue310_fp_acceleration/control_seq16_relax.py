"""Issue #310 Generation 16B: band-5 outer-relaxation discriminator.

Keep the Gen15 measured row-sum-preserving +/-5 electron-Jacobian operator
fixed and vary only the outer Picard relaxation factor.  This isolates whether
the qualified alpha=2/(1+chi_e) is suppressing a useful preconditioned update.
"""
...
base = wall08.base
GENERATED = ROOT / "generated_fp16_relax"
RESULTS = ROOT / "results_fp16_relax"
...
SPECS = (
    {"name": "picard2x_control", "bandwidth": 0, "relaxation_factor": BASE_ALPHA},
    {"name": "band5_alpha002", "bandwidth": 5, "relaxation_factor": 0.02},
    {"name": "band5_alpha005", "bandwidth": 5, "relaxation_factor": 0.05},
    {"name": "band5_alpha010", "bandwidth": 5, "relaxation_factor": 0.10},
    {"name": "band5_alpha020", "bandwidth": 5, "relaxation_factor": 0.20},
)
...
def p0() -> None:
    ...
    print("ISSUE310_GEN16_RELAX_P0: PASS")
    print(json.dumps({"band5": metrics, "alphas": [x["relaxation_factor"] for x in SPECS]}, sort_keys=True))
...
def p1() -> None:
    ...
    cmds = "; ".join(
        f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp16_relax/{n}/input.i"
        for n in CASE_NAMES
    )
    base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; " + cmds)
    print("ISSUE310_GEN16_RELAX_P1: PASS")
...
def p2() -> None:
    ...
    for n in CASE_NAMES:
        for f in ("input.i", "fast_sub.i", "poisson_sub.i"):
            checks.append(
                f"cd /workspace/{rel}/generated_fp16_relax/{n} && "
    ...
    print("ISSUE310_GEN16_RELAX_P2: PASS")
...
def run_case(name: str) -> None:
    ...
    print("ISSUE310_GEN16_RELAX_CASE:", name, result["classification"])
    if code:
...
def aggregate() -> None:
    ...
    summary = {
        ...
        "sequence": 16,
        "classification": (
            "GEN16_RELAX_EVIDENCE_COMPLETE"
            if not missing else "GEN16_RELAX_EVIDENCE_PARTIAL"
        ),
    ...
    RESULTS.mkdir(parents=True, exist_ok=True)
    out = RESULTS / "issue310_gen16_relax_summary.json"
    out.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print("ISSUE310_GEN16_RELAX_AGGREGATE:", summary["classification"])
