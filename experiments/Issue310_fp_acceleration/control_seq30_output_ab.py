"""Issue #310 Gen30: strict matrix A/B of fixed-point diagnostic output cost.

Both 10-heavy-cycle cases preserve the Gen30 scientific/numerical configuration
and native MOOSE -t timing. The only intended input difference is whether the
fp_anchor_csv Output object is enabled. Cases run independently in the governed
matrix; aggregate requires fixed-point and scientific parity before interpreting
any timing difference as diagnostic-output overhead.
"""
...
GENERATED = ROOT / "generated_fp30_output_ab"
RESULTS = ROOT / "results_fp30_output_ab"
...
def p0():
    ...
    # Strict A/B invariant: removing the single enable=false line must make
    # the two electron inputs byte-identical.
    assert trial.replace("    enable = false\n", "", 1) == control
    for fname in ("input.i","poisson_sub.i"):
        assert (GENERATED/"output_on_10hc"/fname).read_bytes() == (GENERATED/"output_off_10hc"/fname).read_bytes()
    ...
    print("ISSUE310_GEN30_P0: PASS", json.dumps({"cases":CASE_NAMES,"strict_ab_only_difference":"fp_anchor_csv enable=false","band5":metrics},sort_keys=True))
...
def p2():
    ...
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp30_output_ab/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq30_output_ab.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp30_output_ab /workspace/{rel}/generated_fp30_output_ab")
    ...
    print("ISSUE310_GEN30_CASE",name,result["classification"])
...
def aggregate():
    ...
    for n,item in found.items():
        cases[n]={k:item.get(k) for k in (
            ...
            "final_mean_electron_energy_avg","csv_file_count","csv_total_bytes",
            "suppress_fp_anchor_output"
        )}

    comparison={}
    gate=False
    if not missing:
        a=found["output_on_10hc"]
        b=found["output_off_10hc"]
        ...
        trajectory={}
        if valid:
            profile=g.wall08._comparison(a,b)
            trajectory=g.seq08._potential_series_parity(a,b)
        numeric_profile=[abs(float(v)) for k,v in profile.items() if isinstance(v,(int,float))]
        max_profile_delta=max(numeric_profile) if numeric_profile else None
        traj_numeric=[abs(float(v)) for k,v in trajectory.items() if k.endswith("_delta") and isinstance(v,(int,float))]
        max_trajectory_delta=max(traj_numeric) if traj_numeric else None
        elapsed_a=float(a.get("elapsed_seconds",0.0))
        elapsed_b=float(b.get("elapsed_seconds",0.0))
        comparison={
            "evidence_valid_both":valid,
            "fixed_point_work_equal":fp_equal,
            "profile_parity":profile,
            "max_profile_metric":max_profile_delta,
            "potential_time_series_parity":trajectory,
            "max_trajectory_abs_delta":max_trajectory_delta,
            "elapsed_seconds_on":elapsed_a,
            "elapsed_seconds_off":elapsed_b,
            "elapsed_speedup_on_over_off":(elapsed_a/elapsed_b if elapsed_b>0 else None),
            "elapsed_reduction_fraction":((elapsed_a-elapsed_b)/elapsed_a if elapsed_a>0 else None),
            "csv_total_bytes_on":a.get("csv_total_bytes"),
            "csv_total_bytes_off":b.get("csv_total_bytes"),
        }
        # Scientific gate is intentionally independent of timing; runner-to-run
        # wall time is evidence only after exact work/profile parity is shown.
        gate=bool(valid and fp_equal and (max_profile_delta is not None and max_profile_delta <= 1.0e-12)
                  and (max_trajectory_delta is not None and max_trajectory_delta <= 1.0e-12))

    summary={
        "issue":310,
        "sequence":30,
        "classification":"GEN30_COMPLETE" if not missing else "GEN30_PARTIAL",
        "missing_cases":missing,
        "cases":cases,
        "comparison":comparison,
        "scientific_parity_gate_pass":gate,
        "reference":{
            "gen29_run":36030285492,
            "head":"d280c7dd58c0aca85319b1e1554bf318a192ebad",
            "delta_phi_abs_tol":1e-6
        },
        "guard":"Strict matrix A/B: output_on and output_off differ only by fp_anchor_csv enable=false. Native -t timing, physics, band5, alpha, Steffensen, timestep, convergence and all other outputs are frozen. Timing interpretation requires the scientific parity gate."
    }
    RESULTS.mkdir(parents=True,exist_ok=True)
    (RESULTS/"issue310_gen30_output_ab_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN30_AGGREGATE",summary["classification"],"scientific_parity_gate_pass=",gate)
