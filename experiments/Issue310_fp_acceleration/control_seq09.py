"""Issue #310 Sequence 09: time-aware native Secant qualification.

Sequence08 established physical parity between the legacy FullSolve/Steady
Poisson path and a time-aware TransientMultiApp/Transient Poisson path.

This matrix freezes the qualified time-aware architecture and compares:
  picard_2x_timeaware : Picard relaxation = 2/(1+chi_e)
...
Every case uses TransientMultiApp + Transient Poisson with no TimeDerivative,
no_restore=true, fast fixed-point algorithm Picard, and transient fixed-point
auto_advance default false. Only the Poisson subapp algorithm and MultiApp
damping vary. Runtime convergence and physical parity are separate.
"""
...
GENERATED = ROOT / "generated_fp09"
RESULTS = ROOT / "results_fp09"
...
ALGORITHM_CASES = (
    ("picard_2x_timeaware", "picard", 2.0 / (1.0 + CHI_E)),
        ...
        json.dumps(
            {
                "issue": 310,
                "sequence": 9,
                "objective": "qualify native Secant damping on the Sequence08-qualified time-aware Poisson architecture",
...
def static_contract() -> dict[str, object]:
    ...
    if len(CASE_NAMES) != 5:
        raise RuntimeError("Sequence09 time-aware Secant matrix must contain exactly five cases")
    ...
    for spec, p in zip(SPECS, built, strict=True):
        ...
        assert math.isclose(float(p["end_time_tau_epsilon_initial"]), FINAL_TAU, rel_tol=0.0, abs_tol=1e-12)
    ...
    return {
        ...
        "cases": built,
        "controlled_changes": ["Poisson fixed_point_algorithm", "MultiApp relaxation_factor"],
    }


def p0() -> None:
    print("ISSUE310_FP09_P0: PASS")
...
def p1() -> None:
    ...
    commands = "; ".join(
        ...
        f"/workspace/{rel}/generated_fp09/{name}/input.i"
...
def p2() -> None:
    ...
    for name in CASE_NAMES:
        checks.append(
            f"cd /workspace/{rel}/generated_fp09/{name} && "
...
def run_case(case_name: str) -> None:
    ...
    base._docker(
        ...
        f"python3 /workspace/{rel}/control_seq09.py --inner-run {case_name}"
    )
    result, code = analyze(case_name)
    (RESULTS / f"{case_name}_result.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print("ISSUE310_FP09_CASE:", case_name, result["classification"])
    if code:
...
def aggregate(root: Path) -> dict[str, object]:
    ...
    reference = found.get("picard_2x_timeaware")
    comparisons: dict[str, object] = {}
    if reference is not None and bool(reference.get("evidence_valid")):
        for name in CASE_NAMES:
            if name == "picard_2x_timeaware" or name not in found:
                continue
            trial = found[name]
            ...
            comparisons[f"{name}_vs_picard_2x_timeaware"] = cmp
    ...
    return {
        "issue": 310,
        "sequence": 9,
        "classification": "TIMEAWARE_SECANT_MATRIX_COMPLETE" if complete else "TIMEAWARE_SECANT_MATRIX_PARTIAL",
        ...
        "cases": found,
        "comparisons_vs_picard_2x_timeaware": comparisons,
        ...
        "interpretation_guard": (
            ...
            "same-run time-aware Picard-2x trajectory/profile, charge and potential. The architecture "
...
def main() -> int:
    ...
    if args.aggregate:
        ...
        (RESULTS / "issue310_fp09_summary.json").write_text(
            json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        print("ISSUE310_FP09_CLASSIFICATION:", summary["classification"])
