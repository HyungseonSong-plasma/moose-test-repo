"""Issue #310 Gen23: 50-heavy-cycle production-scaling wall-time qualification.

Compares the qualified Gen20 band5 alpha=0.45 Picard configuration against
Gen21 Steffensen under 50 heavy cycles / 200 electron steps. Physics, band5
...
GENERATED = ROOT / "generated_fp23_scaling"
RESULTS = ROOT / "results_fp23_scaling"
MEDIUM_HEAVY_CYCLES = 50
MEDIUM_FINAL_TAU = g.FINAL_TAU * MEDIUM_HEAVY_CYCLES
SPECS = (
    {"name":"band5_a045_picard_50hc","bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"picard"},
    {"name":"band5_a045_steffensen_50hc","bandwidth":5,"relaxation_factor":0.45,"custom_convergence":True,"delta_phi_abs_tol":1.0e-6,"fp_algorithm":"steffensen"},
)
...
def p0():
    ...
    assert g.HEAVY_CYCLES == 50
    assert abs(g.FINAL_TAU - 20000.0) < 1e-12
    ...
    print("ISSUE310_GEN23_P0: PASS", json.dumps({"cases":CASE_NAMES,"band5":metrics},sort_keys=True))

def p1():
    build(); rel=ROOT.relative_to(REPO)
    cmds="; ".join(f"python3 /workspace/bin/physics.py preflight /workspace/{rel}/generated_fp23_scaling/{n}/input.i" for n in CASE_NAMES)
    g.base._docker("set -euo pipefail; source /environment; export PYTHONPATH=/workspace; "+cmds)
    print("ISSUE310_GEN23_P1: PASS")

def p2():
    build(); rel=ROOT.relative_to(REPO); checks=[]
    for n in CASE_NAMES:
        for f in ("input.i","fast_sub.i","poisson_sub.i"):
            checks.append(f"cd /workspace/{rel}/generated_fp23_scaling/{n} && /workspace/physics_app/physics-opt --check-input -i {f}")
...
def run_case(name):
    ...
    g.base._docker("set -euo pipefail; source /environment; export MOOSE_DIR=/opt/physics_vendor/moose CRANE_DIR=/opt/physics_vendor/crane SQUIRREL_DIR=/opt/physics_vendor/squirrel ZAPDOS_DIR=/opt/physics_vendor/zapdos METHOD=opt PYTHONPATH=/workspace; "+f"python3 /workspace/{rel}/control_seq23_scaling.py --inner-run {name}; chmod -R a+rwX /workspace/{rel}/results_fp23_scaling /workspace/{rel}/generated_fp23_scaling")
    result,code=g.seq08.analyze(name); raw=next(x for x in SPECS if x["name"]==name)
    result.update(sequence=23,medium_heavy_cycles=MEDIUM_HEAVY_CYCLES,medium_final_tau=MEDIUM_FINAL_TAU,banded_jacobian_width=5,outer_relaxation_factor=raw["relaxation_factor"],fixed_point_algorithm=raw["fp_algorithm"],custom_convergence=True,delta_phi_abs_tol=1e-6,gen19_reference_phi_avg_V=3.9792365725690155,**g._projection_metrics(5),**g._anchor_diagnostic(name))
    ...
    print("ISSUE310_GEN23_CASE",name,result["classification"])
...
def aggregate():
    ...
    summary={"issue":310,"sequence":23,"classification":"GEN23_COMPLETE" if not missing else "GEN23_PARTIAL","missing_cases":missing,"cases":cases,"reference":{"gen19_run":35996399004,"head":"d2d9958735d6afe4c6a23d47880477772a998d78","delta_phi_abs_tol":1e-6,"phi_avg_V":3.9792365725690155},"guard":"50-heavy-cycle production-scaling horizon; Picard vs Steffensen only; band5/alpha/physics/no_restore/delta-phi criterion frozen. Wall time is a first-class promotion metric alongside FP work and corrected-root/trajectory validity."}
    RESULTS.mkdir(parents=True,exist_ok=True); (RESULTS/"issue310_gen23_scaling_summary.json").write_text(json.dumps(summary,indent=2,sort_keys=True)+"\n")
    print("ISSUE310_GEN23_AGGREGATE",summary["classification"])
