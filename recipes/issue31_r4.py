...
transport state, adds physical volume charge and a solved plasma potential,
and deliberately leaves electrostatic transport feedback disabled so the
first Poisson/Gauss-law discriminator is attributable.
...
def _insert_r4_q0_blocks(text: str) -> str:
  ...
  []""",
    ...
    for path in (
        ...
        "FunctorMaterials/r31_poisson_relative_permittivity",
  ...
  []""",
    ...
    text = mb.insert_child_block(
        text,
        "FunctorMaterials",
        """  [r31_poisson_relative_permittivity]
    type = ADGenericFunctorMaterial
    prop_names = 'r31_relative_permittivity'
    prop_values = '1'
    block = plasma
  []""",
    )
    ...
    coeff = r31_relative_permittivity
    block = plasma
  []""",
    )
    text = mb.insert_child_block(
